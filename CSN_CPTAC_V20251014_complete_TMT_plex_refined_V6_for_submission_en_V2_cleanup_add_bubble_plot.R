...
## =========================================================================
## ===== Part 1: GSEA Result Summary CSV Generation =====
## =========================================================================
## Aggregates GSEA results from all datasets into summary CSV files
## Output: proteomic_GSEA/GSEA_result_summary/<stratum>/<predictor>/GSEA_summary_H.csv

generate_gsea_summary_csv <- function(
  gsea_prefix = GSEA_PREFIX %||% "proteomic_GSEA",
  datasets = c(
    "brca_cptac_2020", "coad_cptac_2019", "gbm_cptac_2021",
    "luad_cptac_2020", "lusc_cptac_2021", "paad_cptac_2021", 
    "ucec_cptac_2020"
  ),
  strata = c("ALL", "TP53_mutant", "TP53_wild_type"),
  predictors = NULL,
  ...
  log_msg("===== Starting GSEA Result Summary CSV Generation =====")

  ## Auto-detect predictors from first valid dataset if not specified
  if (is.null(predictors)) {
    for (ds in datasets) {
      test_dir <- file.path(gsea_prefix, collection, "BatchAdj", ds, strata[1])
      if (dir.exists(test_dir)) {
        pred_dirs <- list.dirs(test_dir, recursive = FALSE, full.names = FALSE)
        pred_dirs <- pred_dirs[pred_dirs != "summary"]
        if (length(pred_dirs) > 0) {
          predictors <- pred_dirs
          log_msg("Auto-detected predictors: %s", paste(predictors, collapse = ", "))
          break
        }
      }
    }
  ...
  if (is.null(predictors) || length(predictors) == 0) {
    log_msg("[ERROR] No predictors found. Please check the directory structure.")
    return(invisible(NULL))
  }

  ## Create output directory structure
  out_base <- file.path(gsea_prefix, "GSEA_result_summary")

  for (stratum in strata) {
    for (predictor in predictors) {
      out_dir <- file.path(out_base, stratum, predictor)
      dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

      ## Collect GSEA results from all datasets
      results_list <- list()

      for (ds in datasets) {
        csv_path <- file.path(gsea_prefix, collection, "BatchAdj", ds, stratum, predictor, "GSEA_limma_t_cont.csv")
        ...
        if (file.exists(csv_path)) {
          ...
          if (!is.null(dt) && nrow(dt) > 0) {
            ## Extract pathway, NES, padj columns
            if (all(c("pathway", "NES", "padj") %in% names(dt))) {
              df_sub <- data.frame(
                pathway = dt$pathway,
                NES = dt$NES,
                padj = dt$padj,
      ...
      if (length(results_list) == 0) {
        log_msg("  [Skip] %s | %s: No data found", stratum, predictor)
        next
      }
      ...
      ## Pivot to wide format: rows = datasets, columns = genesets (NES and padj for each)
      nes_wide <- combined %>%
        dplyr::select(dataset, pathway, NES) %>%
        tidyr::pivot_wider(names_from = pathway, values_from = NES, names_prefix = "NES_")

      padj_wide <- combined %>%
        dplyr::select(dataset, pathway, padj) %>%
        tidyr::pivot_wider(names_from = pathway, values_from = padj, names_prefix = "padj_")

      ## Merge NES and padj tables
      summary_df <- dplyr::left_join(nes_wide, padj_wide, by = "dataset")

      ## Reorder datasets according to input order
      summary_df$dataset <- factor(summary_df$dataset, levels = datasets)
      summary_df <- summary_df %>% dplyr::arrange(dataset)
      summary_df$dataset <- as.character(summary_df$dataset)

      ## Write output
      out_file <- file.path(out_dir, paste0("GSEA_summary_", collection, ".csv"))
      data.table::fwrite(summary_df, out_file)
      log_msg("  [Saved] %s | %s -> %s", stratum, predictor, out_file)
    }
  }

  log_msg("===== GSEA Result Summary CSV Generation Complete =====")
  invisible(TRUE)
}

## Execute GSEA Summary CSV Generation
generate_gsea_summary_csv()


## =========================================================================
## ===== Part 2: GSEA Bubble Plot Generation =====
## =========================================================================
## Creates publication-quality bubble plots for GSEA results using ggplot2
## Output: proteomic_GSEA/bubble_plot/<prefix>_<stratum>_<predictor>.tiff/.pdf
...
generate_gsea_bubble_plot <- function(
  ...
  log_msg("  Stratum: %s | Predictor: %s", stratum, predictor)
  log_msg("  Gene sets: %s", paste(genesets, collapse = ", "))
  ...
  for (ds in datasets) {
    ...
    if (file.exists(csv_path)) {
      ...
      log_msg("  [Warning] File not found: %s", csv_path)
    }
  }

  if (nrow(plot_data) == 0) {
    log_msg("[ERROR] No data found for bubble plot. Please check parameters.")
    return(invisible(NULL))
  }
  ...
  out_dir <- file.path(gsea_prefix, "bubble_plot")
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  ## Generate output filename
  out_base <- file.path(out_dir, sprintf("%s_%s_%s", output_prefix, stratum, predictor))
  ...
  log_msg("  [Saved] TIFF: %s", tiff_file)
  ...
  log_msg("  [Saved] PDF: %s", pdf_file)

  log_msg("===== GSEA Bubble Plot Generation Complete =====")
  invisible(list(plot = p, tiff = tiff_file, pdf = pdf_file))
}

## Execute GSEA Bubble Plot Generation (default: ALL stratum, CSN_SCORE predictor)

      ...
      for (sc in colnames(Y)) {
        ...
        for (pn in names(predictors)) {
          ...
          if (isTRUE(make_plots)) {
            ...
            p1  <- ggplot2::ggplot(dfp, ggplot2::aes(x = x, y = y)) +
              ggplot2::geom_point(size = 1) +
              ggplot2::geom_smooth(method = "lm", se = FALSE) +
              ggplot2::labs(title = sprintf("%s vs %s (%s/%s/%s)", sc, pn, ds_id, st_name, ver),
                            x = sprintf("%s (residualized)", pn),
                            y = sprintf("%s (residualized)", sc)) +
              ggplot2::theme_bw(base_size = 10)
            ggplot2::ggsave(filename = file.path(plot_dir, sprintf("%s_vs_%s_%s_%s.png", sc, pn, st_name, ver)),
                            plot = p1, width = 4, height = 3, dpi = 300, bg = "white")
          }
        } # end predictors
      }   # end scores

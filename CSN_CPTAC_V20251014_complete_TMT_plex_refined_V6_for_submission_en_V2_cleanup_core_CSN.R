...
build_csn_score_safe <- function(mat0, subunits, combine_7AB = TRUE,
  ...
  cs_try <- try(
    {
      # Filter-out COPS7A and COPS7B for CSN_SCORE PC1 calculation
      sub_core <- setdiff(sub, c("COPS7A", "COPS7B"))
      build_csn_score(mat0, subunits = sub_core, combine_7AB = combine_7AB, min_members = min_members)
    },
    silent = TRUE
  )
  ...
  sub_core <- setdiff(sub, c("COPS7A", "COPS7B"))
  X <- .clean_for_pca(mat0[sub_core, , drop = FALSE], min_samples = pca_min_samples, min_genes = min_members)
  if (is.null(X)) {
...
audit_csn_score_feasibility_safe <- function(ds_id, stratum, mat0, present_sub,
  ...
  sub_core <- setdiff(intersect(present_sub, rownames(mat0)), c("COPS7A", "COPS7B"))
  X <- .clean_for_pca(mat0[sub_core, , drop = FALSE],
...
GSEA_PREFIX <- "proteomic_GSEA_core"
...
audit_csn_score_feasibility <- function(ds_id, stratum, mat0, present_sub,
  ...
  core_sub <- setdiff(present_sub, c("COPS7A", "COPS7B"))
  ...
  if (isTRUE(csn_can_pca)) {
    ...
    core_sub <- setdiff(present_sub, c("COPS7A", "COPS7B"))
    Z <- do.call(rbind, lapply(core_sub, function(g) get_z(mat0[g, ok_sam, drop = FALSE])))
    rownames(Z) <- core_sub
...
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group) {
  ...
  ## 4) Perform analysis on a per-prediction vector basis: original value of subunit, CSN_SCORE, RESIDUAL_<SU>
  {
    ...
    csn_score <- build_csn_score_safe(
      mat0,
      subunits = core_sub, combine_7AB = TRUE,
      min_members = 5L, pca_min_samples = 10L
    )

...
## =========================================================
## [NEW] BRCA_immune_analysis：只對 brca_cptac_2020 做免疫/基質分數 × predictors 相關
## - 來源：data_clinical_patient.txt（5 欄）
## - predictors：CSN subunits 的 protein abundance + CSN_SCORE + RESIDUAL_<subunit>
## - 方法：Pearson/Spearman；RAW 與 BatchAdj 共變項策略同 protein 版 GSEA
## - 分層：ALL / TP53_mutant / TP53_wild_type；另做 ALL 的 TP53 interaction
## - 輸出：<ds_dir>/BRCA_immune_analysis/<stratum>/<version>/*.csv
## =========================================================
...
.read_brca_scores_from_patient <- function(ds_dir, sample_ids){
  ...
  if (!file.exists(pat_fp)) {
    message("[BRCA-immune] 找不到 data_clinical_patient.txt：", ds_dir)
  ...
  keep_cols <- c("ESTIMATE_IMMUNE_SCORE","ESTIMATE_STROMAL_SCORE",
                 "XCELL_IMMUNE_SCORE","XCELL_STROMAL_SCORE","CIBERSORT_ABSOLUTE_SCORE")
  out <- matrix(NA_real_, nrow = length(sample_ids), ncol = length(keep_cols),
...
.residualize_pair <- function(x, y, batch = NULL, covars = NULL, min_n = 8L){
  # 若全域已定義 residualize_to_covars，沿用；否則用 orthogonalize_to 作為後備
...
.run_brca_immune_cor <- function(dataset_dirs_map = NULL, make_plots = FALSE, min_pairs = 8L){
  ...
  if (is.null(ds_dir) || !dir.exists(ds_dir)) {
    message("[BRCA-immune] 找不到 brca_cptac_2020 目錄，略過")
    return(invisible(NULL))
  }
  
  out_root <- file.path(ds_dir, "BRCA_immune_analysis")
  dir.create(out_root, recursive = TRUE, showWarnings = FALSE)
  
  # [NEW] 繪圖篩選與輸出格式（可由全域變數或 options 覆蓋）
  plot_filters  <- get0("BRCA_PLOT_FILTERS", ifnotfound = list(
    stratum   = NULL,  # 例：c("ALL","TP53_mutant","TP53_wild_type")
    version   = NULL,  # 例：c("RAW","BatchAdj")
    score     = NULL,  # 例：c("ESTIMATE_IMMUNE_SCORE","XCELL_IMMUNE_SCORE", ...)
    predictor = NULL,  # 例：c("CSN_SCORE","GPS1","RESIDUAL_COPS5", ...)
    method    = NULL   # 例：c("pearson","spearman")
  ))
  # 預設只輸出 .tiff；可改為 c("TIFF","PNG","JPG","PDF")
  plot_formats <- toupper(get0("BRCA_PLOT_FORMATS", ifnotfound = c("TIFF")))
  plot_dpi     <- get0("BRCA_PLOT_DPI",    ifnotfound = 600L)  # 投稿常用 600 dpi
  ...
  CELL_PALETTES <- list(
    # 原本四組
    CELL_BLUE        = list(pt = "#2D2D2D", ln = "#1F77B4"),  # 深灰點 / 藍線
    CELL_TEAL        = list(pt = "#2D2D2D", ln = "#1B9E77"),  # 深灰點 / 青綠線
    CELL_ORANGE      = list(pt = "#2D2D2D", ln = "#D55E00"),  # 深灰點 / 橘線
    CELL_PURPLE      = list(pt = "#2D2D2D", ln = "#6A3D9A"),  # 深灰點 / 紫線
    # 新增：更鮮豔明亮系列（給回歸線顏色 ln；點預設仍為深灰 pt，可另外覆蓋）
  ...
  # 可用 BRCA_PLOT_COLORSET 全域變數覆蓋（例如 "CELL_TEAL"）
  ...
  # Cell-like 主題：無外框、細黑軸線、小字體；所有標題文字縮小且不覆蓋到圖表（位於圖外）
  .cell_theme <- function(base_size = 10){
    # 可由全域變數覆蓋字級；未設定則用預設比例
    ...
    ggplot2::theme_classic(base_size = base_size) +
      ggplot2::theme(
        panel.border  = ggplot2::element_blank(),                      # 不要外框
        axis.line     = ggplot2::element_line(linewidth = 0.6, colour = "black"),
        axis.ticks    = ggplot2::element_line(linewidth = 0.6, colour = "black"),
        plot.title    = ggplot2::element_text(face = "bold", size = title_sz,
                                              margin = ggplot2::margin(b = 2)),
        plot.subtitle = ggplot2::element_text(size = subtitle_sz,
  ...
  # 只保留 CSN subunits 存在者，並準備 score 資料
  present_sub <- intersect(csn_subunits, rownames(mat0_full))
  if (!length(present_sub)) {
    message("[BRCA-immune] 此資料集缺少 CSN subunits，略過")
    return(invisible(NULL))
  }
  ...
  for (st_name in names(strata_list)) {
    ...
    if (length(keep) < 4L) {
      message(sprintf("[BRCA-immune][%s] 樣本過少(%d)，略過", st_name, length(keep)))
      next
    }
    mat0 <- mat0_full[, keep, drop = FALSE]
    # CSN_SCORE（以分層樣本計算）
    ...
    for (ver in versions) {
      ...
      # 準備 predictors：各 subunit、CSN_SCORE、RESIDUAL_<subunit>
      predictors <- list()
      for (su in present_sub) {
      ...
      # [NEW] 收集繪圖工作；待 BH 調整完成後再實際出圖（可正確標示 padj）
      plot_jobs <- list()
      ...
      for (sc in colnames(Y)) {
        ...
        for (pn in names(predictors)) {
          ...
          # 殘差化後做 Pearson / Spearman
          xy <- .residualize_pair(x0[ok], y0[ok], batch = batch_use[ok], covars = as.data.frame(cov_use[ok, , drop = FALSE]))
          # Pearson
          cp <- suppressWarnings(stats::cor.test(xy$x, xy$y, method = "pearson", exact = FALSE))
          lm_fit <- try(stats::lm(xy$y ~ xy$x), silent = TRUE)
          intercept_p <- if (!inherits(lm_fit, "try-error")) unname(coef(lm_fit)[1]) else NA_real_
          slope_p     <- if (!inherits(lm_fit, "try-error")) unname(coef(lm_fit)[2]) else NA_real_
          r2_p        <- as.numeric(cp$estimate)^2
          eq_p        <- sprintf("y = %.6f + %.6f * x", intercept_p, slope_p)
          
          # Spearman（以秩做 OLS 只為了提供方程式；r^2 仍為 rho^2）
          cs <- suppressWarnings(stats::cor.test(xy$x, xy$y, method = "spearman", exact = FALSE))
          rx <- rank(xy$x); ry <- rank(xy$y)
          lm_rs <- try(stats::lm(ry ~ rx), silent = TRUE)
          intercept_s <- if (!inherits(lm_rs, "try-error")) unname(coef(lm_rs)[1]) else NA_real_
          slope_s     <- if (!inherits(lm_rs, "try-error")) unname(coef(lm_rs)[2]) else NA_real_
          r2_s        <- as.numeric(cs$estimate)^2
          ...
          rows[[length(rows)+1L]] <- data.frame(
            ...
            r = as.numeric(cp$estimate), p = as.numeric(cp$p.value),
          ...
          rows[[length(rows)+1L]] <- data.frame(
            ...
            r = as.numeric(cs$estimate), p = as.numeric(cs$p.value),
            padj = NA_real_, R2 = r2_s, equation = eq_s,
            stringsAsFactors = FALSE, check.names = FALSE
          )
          
          # （可選）散點圖：改為收集繪圖工作（避免當下尚未有 padj）
            ...
            if (want_stratum && want_version && want_score && want_predictor) {
              ...
              if (length(methods_want)) {
                for (mm in methods_want) {
                  rr <- if (mm == "pearson") as.numeric(cp$estimate) else as.numeric(cs$estimate)
                  pp <- if (mm == "pearson") as.numeric(cp$p.value)  else as.numeric(cs$p.value)
                  plot_jobs[[length(plot_jobs)+1L]] <- list(
                    dataset   = ds_id, stratum = st_name, version = ver,
                    score     = sc, predictor = pn, method = mm,
                    r = rr, p = pp,
                    R2 = if (mm == "pearson") r2_p else r2_s,
                    equation  = if (mm == "pearson") eq_p else eq_s,
                    df = dfp, n = sum(ok)
                  )
                }
              }
      ...
      if (length(rows)) {
        ...
        # 依（dataset × stratum × version × method × score）分組做 BH
        out_tbl <- split(
          out_tbl,
          list(out_tbl$dataset, out_tbl$stratum, out_tbl$version, out_tbl$method, out_tbl$score),
          drop = TRUE
        )
        out_tbl <- lapply(out_tbl, function(d) { d$padj <- stats::p.adjust(d$p, method = "BH"); d })
        # [NEW] 依 out_tbl 的 BH 調整結果作圖（Cell 風格；含 CI；副標示 r 與 padj；不覆蓋到圖表）
        if (isTRUE(make_plots) && length(plot_jobs)) {
          ...
          for (job in plot_jobs) {
            # 再檢查 method 篩選（若外部變動）
            if (!is.null(plot_filters$method) &&
                !(tolower(job$method) %in% tolower(plot_filters$method))) next
            ...
            sub_txt   <- sprintf("%s | %s / %s / %s | r=%.3f, padj=%s, n=%d",
        ...
        message(sprintf("[BRCA-immune][%s/%s] 已輸出 %d 筆結果 → %s",
        ...
        message(sprintf("[BRCA-immune][%s/%s] 沒有任何合格配對（rows=0）；已輸出空檔案。",
                        st_name, ver))
      }
    }
    
    ## ---- TP53 interaction（僅 ALL；同 GSEA 流程精神） ----
    if (identical(st_name, "ALL")) {
      ...
      for (ver in versions) {
        ...
        rows_i <- list()
        for (sc in colnames(Y)) {
          y0 <- suppressWarnings(as.numeric(Y[[sc]])); names(y0) <- keep
          for (pn in names(predictors)) {
            x0 <- predictors[[pn]]; names(x0) <- keep
            ok <- is.finite(x0) & is.finite(y0) & is.finite(tp53_num)
            if (sum(ok) < min_pairs) next
            dfm <- data.frame(y = y0[ok], x = x0[ok], tp53 = tp53_num[ok],
                              cov_use[ok, , drop = FALSE])
            if (!is.null(batch_use)) dfm$batch <- droplevels(batch_use[ok])
            # y ~ x * tp53 + covariates (+ batch)
            form <- as.formula(paste0("y ~ x * tp53 + ",
            ...
            eq_line  <- paste0("y = ", sprintf("%.6f", unname(coef(fit)[1])),
        ...
        if (length(rows_i)) {
          ...
          message(sprintf("[BRCA-immune][ALL/%s] 互動項：已輸出 %d 筆 → %s",
                          ver, nrow(out_i),
          ...
          message(sprintf("[BRCA-immune][ALL/%s] 互動項：無合格結果，已輸出空檔案。", ver))
          
        }
      }
    } # end interaction
  }   # end strata
  message("[BRCA-immune] 完成：輸出 → ", out_root)
}

BRCA_TITLE_SIZE       <- 8   # 主標題
BRCA_SUBTITLE_SIZE    <- 5    # 副標題（放 r 與 padj）
BRCA_AXIS_TITLE_SIZE  <- 8    # x / y 軸標題
BRCA_AXIS_TEXT_SIZE   <- 8    # 刻度數字


BRCA_PLOT_FILTERS <- list(
  stratum   = c("ALL"),  # 需者擇一或數個
  version   = c("RAW","BatchAdj"),
  score     = c("CIBERSORT_ABSOLUTE_SCORE"),
  predictor = c("CSN_SCORE","COPS7A", "COPS7B"),    # 範例
  method    = c("pearson")
)
...
# 選擇配色（線色）：更鮮豔明亮
#BRCA_PLOT_COLORSET <- "CELL_BRIGHT_BLUE"   # 或 CELL_BRIGHT_BLUE / CELL_AQUA / CELL_SCARLET / CELL_GOLD / CELL_MINT
...
# 調整散點大小與顏色（可選）
BRCA_POINT_SIZE  <- 0.9
BRCA_POINT_COLOR <- "#111111"          # 若不指定，預設沿用色盤 pt（深灰）


BRCA_PLOT_FORMATS <- c("TIFF")     # 預設即為 tiff 可省略；若想多格式：c("TIFF","PNG","JPG","PDF")
#BRCA_PLOT_FORMATS <- c("TIFF","PNG","JPG","PDF")  # 要全部就像這樣
BRCA_PLOT_DPI     <- 600


# 然後照你原流程呼叫（若要出圖則 make_plots=TRUE，並在 BRCA_PLOT_FILTERS 指定想畫的條件）



## ===== [NEW] 立即執行 BRCA_immune_analysis（只跑 brca_cptac_2020；預設不繪圖） =====
tryCatch(
  {
    .run_brca_immune_cor(
  ...
  error = function(e) {
    message("[BRCA-immune] 執行失敗：", conditionMessage(e))
    # 簡易呼叫堆疊（避免 R 的 traceback 在 tryCatch 裡不好用）
    calls <- sys.calls()
    message("[BRCA-immune] 呼叫堆疊：",
            paste(utils::capture.output(print(calls)), collapse = " | "))
    stop(e)  # 明確拋出，讓外層流程看得到錯誤
  }
)

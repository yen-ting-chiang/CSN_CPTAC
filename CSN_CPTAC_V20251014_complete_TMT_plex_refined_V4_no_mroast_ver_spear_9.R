...
audit_covars_coverage <- function(tag, ds_id, stratum, su,
  ...
  line <- sprintf("  [audit:%s] covars={%s} | coverage: purity=%.1f%%, sex=%.1f%%, age=%.1f%% | batch_levels=%s",
                  tag,
  ...
  df <- data.frame(
    ...
    batch_sizes  = if (!is.null(batch)) paste(sprintf("%s=%d", names(sort(table(batch), decreasing=TRUE)),
                                                      as.integer(sort(table(batch), decreasing=TRUE))), collapse="; ")
    else NA_character_,
...
if (!is.null(COMBO_MODE)) {
  ...
  ## 強制本輪只跑 limma_t
  PIPELINES_TO_RUN <<- c("limma_t"); .RUN_LIMMA <<- TRUE
...
run_predictor_analyses <- function(
  ...
  pred <- pred_all[fin]                  # ← 原始（未中心化）predictor
  ...
  ## NEW: 取這一輪會真正用到的欄位（predictor + 已選 covars）
  use_cols <- unique(c(
    "predictor",
  ...
  if (.RUN_LIMMA) {
    for (grp_name in names(genesets_by_group)) {
      ...
      ## [NEW] collection-first root: <collection>/<dataset>/<stratum>/<BatchAdj>/<predictor>
      ...
      if ("BatchAdj" %in% getOption("csn.run_passes", c("BatchAdj"))) {
        ...
        if (!is.null(t2)) {
          ...
          ## --- Add back: TP53 interaction (BatchAdj; A2) ---
            ...
            if (nlevels(droplevels(tp53_ba_fac)) >= 2) {
              ...
              if (.RUN_LIMMA) {
                ...
                if (coef_int %in% colnames(coef(fit_int_ba))) {
                  ...
                  ## 與舊版相同：逐 collection 做 GSEA 並寫 GSEA_limma_interaction.csv
                  res_fg <- gsea_from(pw, ranks)
                  if (!is.null(res_fg) && nrow(res_fg)) {
                    data.table::fwrite(as.data.frame(res_fg),
...
## =========================================================
## ===== 每個 stratum：BatchAdj（固定版本）
## =========================================================
## ===== run_one_stratum：統一檔名、寫 sample_counts、批次備註補強（2,3,7）=====
...
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group){
  ...
  ## 2) limma 用的插補+過濾矩陣 + CSN_SCORE 可行性稽核
  ...
  # BatchAdj：limma_t_cont  + interaction（只彙整現有 csv，不重跑）
  for (grp_name in names(genesets_by_group)) {
    out_root_coll <- file.path(COMBO_PREFIX %||% "csn_gsea_results_TP53_by_collection", safe_fs_name(grp_name), ds_id, basename(out_root))
...
for (ds in names(dataset_dirs_run)) {
  ...
  ## 依序跑三個 strata（僅 BatchAdj；已在 run_one_stratum 裡寫好）
  if (.should_run("ALL")) {
    run_one_stratum(
...
csn_pairwise_correlation_one_ds <- function(
  ...
  cov_raw <- cov0 # 若未定義 RAW_covars 區，先就地定義供下游沿用
  ...
  # Dataset-level 插補矩陣（與特定 predictor 無關）
  ...
  for (p in pairs) {
    ...
    ## ---- 版本 3: BatchAdj_covars----
    cov_ba <- cov_raw # 沿用共變項選擇
...
## - 僅 BatchAdj（含 limma_t 與 interaction）
...
run_gsea_heatmaps_for_all_datasets <- function(dataset_dirs_map = NULL,
  ...
  for (ds in names(dataset_dirs_map)) {
    ...
    for (csv in csvs) {
      ...
      # 僅限 limma_t_cont（ordered 版本僅對連續版適用）
...
# 解析 meta_fdr/summary 路徑，取得 version=BatchAdj 與 stratum
...
run_meta_fdr_heatmaps <- function(root = if (is.null(COMBO_PREFIX)) "csn_gsea_pan_summary_TP53/meta_fdr/summary" else file.path(COMBO_PREFIX, "csn_gsea_pan_summary_TP53/meta_fdr/summary")) {
  ...
  for (csv in csvs) {
    ...
    if (is_interaction_all && identical(tolower(meta$stratum), "all")) {
      dir_now    <- normalizePath(dirname(csv), winslash = "/", mustWork = FALSE)
      parent_dir <- dirname(dir_now)  # e.g. .../meta_fdr/summary/ALL/BatchAdj/H

...
## ---- [NEW] protein loader for predictors（不覆寫 RNA 版）----
load_matrix_from_dataset_dir_protein <- function(dir){
  fp <- file.path(dir, "data_protein_quantification.txt")
  if (!file.exists(fp)) stop(glue::glue("找不到檔案：{fp}"))
  log_msg("讀取 protein 矩陣（predictors 用）：{basename(fp)}")
  dat <- suppressMessages(readr::read_tsv(fp, guess_max = 200000, show_col_types = FALSE))
  gene_cols <- c("Hugo_Symbol","hugo_symbol","Gene","Gene_Symbol","HugoSymbol","GENE_SYMBOL","gene","gene_symbol")
  gcol <- intersect(gene_cols, names(dat)); if (!length(gcol)) gcol <- names(dat)[1]
  dat  <- dplyr::rename(dat, Gene = !!gcol[1]); dat$Gene <- sub("\\|.*$", "", dat$Gene)
  ...
  log_msg("predictor-protein 矩陣維度：%d genes × %d samples", nrow(m), ncol(m))
  m
}
...
COMBO_PREFIX <- if (!is.null(COMBO_MODE)) paste0("mRNA_prot_predictors_", COMBO_MODE) else NULL
...
run_imputation_sensitivity_demo <- function(ds_id, ds_dir,
  ...
  prot0  <- load_matrix_from_dataset_dir_protein(ds_dir)
  common <- intersect(colnames(mat0), colnames(prot0))
  mat0   <- mat0[,  common, drop = FALSE]
  prot   <- prot0[, common, drop = FALSE]
  if (predictor_name == "CSN_SCORE") {
    x_vec <- build_csn_score(prot, subunits = present_sub, combine_7AB = TRUE, min_members = 5L)
    ...
    x_vec <- residualize_vector(prot[su, ], build_csn_score(prot, present_sub),
                                batch = {bi <- get_batch_factor(ds_dir, colnames(mat0)); if (is.null(bi)) NULL else bi$fac},
                                covars = base_covars_all, min_n = 8L)
    exclude_genes <- su
  } else {
    stopifnot(predictor_name %in% rownames(prot))
    x_vec <- prot[predictor_name, ]
  ...
  if (!is.null(tbl) && nrow(tbl)) {
    ...
    data.table::fwrite(tbl, file.path(out_dir, sprintf("impute_sensitivity_%s_%s.csv", safe_fs_name(geneset_group), predictor_name)))
    ...
    openxlsx::saveWorkbook(
      wb,
      file.path(out_dir, sprintf("impute_sensitivity_%s_%s.xlsx", safe_fs_name(geneset_group), predictor_name)),
      overwrite = TRUE
    )
...
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group){
  ...
  {
    ## [NEW] predictors 來源改為 protein，並與 RNA 對齊樣本
    ...
    if (length(common_sam) < (2 * min_per_group)) {
      log_msg("  [跳過] RNA 與 protein 樣本交集不足：%d", length(common_sam))
      return(invisible(NULL))
    }
    ...
    if (!length(present_sub)) { log_msg("  [跳過] protein 中沒有任何 CSN subunit"); return(invisible(NULL)) }
    
    ## 4a) 逐 subunit（predictor = protein；排名時排除自身 gene）
    ...
    ## 4b) CSN complex score（predictor = protein PC1；排名時排除所有 CSN 成員）
    csn_score_prot <- build_csn_score_safe(
      prot_pred, subunits = present_sub, combine_7AB = TRUE,
      min_members = 5L, pca_min_samples = 10L
    )
    if (sum(is.finite(csn_score_prot)) >= (2 * min_per_group)) {
      ...
      log_msg("  [CSN_SCORE] protein PC1 非 NA 樣本不足，略過")
    }
    
    ## 4c) RESIDUAL_<SU>（以 protein CSN_SCORE + 共變項殘差化 predictor，再做分析）
    for (su in present_sub) {
      y <- prot_pred[su, , drop = TRUE]
      y_res <- residualize_vector(y = y, csn_score = csn_score_prot,
                                  batch = batch_all, covars = sa_all_limma, min_n = min_per_group)
      run_predictor_analyses(
...
csn_pairwise_correlation_one_ds <- function(
  ...
  # RNA 版：不使用 MinProb（proteomics 專用）；暫不插補，後續會以 row-median 修補缺失
  Mimp <- Mimp
  ...
  if (.use_no_batch_sva(ds_id) && exists("estimate_svs", mode = "function")) {
    ...
    if (!inherits(sv, "try-error") && !is.null(sv)) {
      ...
      # 將 SV 統一成「samples × k」
      if (nrow(S) == ncol(Mimp)) {
        # 目前 S 的 row 數等於樣本數，直接視為 samples × k
        if (is.null(rownames(S)) || length(rownames(S)) != nrow(S)) {
          rownames(S) <- colnames(Mimp)
        }
      } else if (ncol(S) == ncol(Mimp)) {
        # 目前 S 的 col 數等於樣本數，說明是 k × samples → 轉置
        S <- t(S)
        rownames(S) <- colnames(Mimp)
      } else {
        log_msg("[SVA%s] unexpected SV dims: %d×%d vs samples=%d -> SV=NULL",
                sprintf("%s-pair", ds_id), nrow(S), ncol(S), ncol(Mimp))
        S <- NULL
      }
      if (!is.null(S) && nrow(S) == ncol(Mimp)) {
        sv_df <- as.data.frame(S)
  ...
  # RNA 版：確保 Mimp 無缺失（行中位數補缺），避免 prcomp 出錯
  if (any(!is.finite(Mimp))) {
    for (i in seq_len(nrow(Mimp))) {
      v <- Mimp[i, ]
      if (any(!is.finite(v))) {
...
##   - 將各 dataset 的 mRNA 矩陣（FPKM/RPKM）轉為 TPM（每一個 sample 獨立：
##       TPM_ij = 10^6 * FPKM_ij / Σ_k FPKM_kj ；RPKM 同式）
##   - 針對指定 CSN subunits：在每個 dataset 內對樣本取匯總（預設 median）
##   - 輸出：TPM/CSN_subunits_TPM_<agg>_<metric>.csv
##   - 繪圖：x=dataset (可指定與排序)、y=subunits(固定順序)、色階=TPM 或 log2(TPM+1)
##   - 色系可選（預設 "Blues"）；預設只輸出 .tiff
## =========================================================
suppressPackageStartupMessages({
  if (!requireNamespace("ggplot2", quietly = TRUE)) install.packages("ggplot2", repos = "https://cloud.r-project.org")
  if (!requireNamespace("dplyr", quietly = TRUE)) install.packages("dplyr", repos = "https://cloud.r-project.org")
...
# ---- FPKM/RPKM -> TPM：對每個 sample (column) 做 per-column re-scale 使 colSums=1e6 ----
...
# ---- 殘差化：跟你 GSEA 使用的一樣（不含 SVA；僅 sex/age/purity）----
.residualize_y_by_covars <- function(y, covars){
  # 若你已有 residualize_to_covars()，沿用；否則提供最小實作
  if (exists("residualize_to_covars", mode = "function")) {
    ...
    # 先丟掉全 NA 與常數/單一水準欄，避免 complete.cases 全滅
    if (ncol(DF) > 0) {
      all_na <- vapply(DF, function(v) all(is.na(v)), logical(1))
      if (any(all_na)) DF <- DF[, !all_na, drop = FALSE]
      is_const <- vapply(DF, function(v) {
    ...
    # 若清完後沒有任何可用 covariate，就用截距-only；否則用 1 + .
    yv <- suppressWarnings(as.numeric(y))
    ok <- is.finite(yv) & (if (ncol(DF) == 0) TRUE else stats::complete.cases(DF))
    if (sum(ok) < 8L) return(setNames(rep(NA_real_, length(y)), names(y)))
  ...
  # 為了保留尺度，讓校正後的向量回到「原本的平均」：residual + mean(y)
  mu <- suppressWarnings(mean(as.numeric(y[is.finite(y)]), na.rm = TRUE))
  r + ifelse(is.finite(mu), mu, 0)
}

# ---- 建 subunits × datasets 的 TPM 匯總矩陣（預設各 dataset 內取 median）----
build_csn_subunits_tpm_matrix <- function(dataset_dirs_map,
  ...
  agg <- match.arg(agg)
  if (is.null(dataset_dirs_map)) {
    if (exists("dataset_dirs_run")) dataset_dirs_map <- get("dataset_dirs_run", inherits = TRUE)
    else if (exists("dataset_dirs")) dataset_dirs_map <- get("dataset_dirs", inherits = TRUE)
    else stop("找不到 dataset 目錄對應（dataset_dirs_run / dataset_dirs）")
  }
  # 篩選與排序 datasets
  ...
  if (!length(keep)) stop("datasets_to_plot 和可用 datasets 無交集")
  
  # 收集每個 dataset 的 subunit 匯總值
  out_list <- list()
  for (ds in keep) {
    ...
    if (isTRUE(adjust_covars)) {
      ...
      # 逐基因殘差化（僅 sex/age/purity；不含 SVA），再做匯總
      adj_mat <- sub_mat
      for (g in rownames(sub_mat)) {
        y <- as.numeric(sub_mat[g, sam_all]); names(y) <- sam_all
  ...
  if (!length(out_list)) stop("沒有任何 dataset 產生 TPM 匯總值")
  # 對齊 rownames & 合併
  all_rows <- unique(unlist(lapply(out_list, names)))
  all_rows <- intersect(subunits, all_rows)  # 只留 CSN subunits 並保序
...
.get_palette <- function(name, n = 256){
  ...
  ## Cell journal 綠-黃色系：低=淡黃、高=適中綠（避免太深近黑）
  ...
  if (requireNamespace("RColorBrewer", quietly = TRUE)) {
    bnm <- switch(nm,
                  ...
                  purples = "Purples", greys = "Greys", oranges = "Oranges", "Blues")
...
plot_csn_subunits_tpm_heatmap <- function(MAT, metric = "log2TPM",
  ...
  p <- ggplot(df, aes(x = dataset, y = subunit, fill = value)) +
    ...
    scale_y_discrete(limits = rev(levels(df$subunit))) +  # 由上到下顯示 CSN_SUB_ORDER
  ...
  base <- file.path(out_dir, sprintf("CSN_subunits_%s_heatmap_%s%s",
                                     metric, .safe_fs(pal),
...
# ---- 一鍵：建 TPM 矩陣 + 存 CSV + 畫圖 ----
run_csn_subunits_TPM <- function(dataset_dirs_map = NULL,
                                 subunits = CSN_SUB_ORDER,
                                 datasets_to_plot = NULL,    # 想畫哪些 dataset 以及順序（字串向量）
                                 agg = c("median","mean"),
                                 use_log2 = TRUE,
                                 pal = "cell",
                                 label_digits = 1,
  ...
  res_raw <- build_csn_subunits_tpm_matrix(dataset_dirs_map, subunits, match.arg(agg),
                                           datasets_to_plot, use_log2, adjust_covars = FALSE)
  MAT_raw <- res_raw$mat
  ...
  base_raw <- plot_csn_subunits_tpm_heatmap(MAT_raw,
                                            ...
                                            name_suffix = "RAW")
  message("[TPM] RAW 矩陣 CSV：", out_csv_raw)
  message("[TPM] RAW 圖檔 base：", base_raw)
  
  ## ---- AdjCovars（sex/age/purity；不含 SVA）版本 ----
  res_adj <- build_csn_subunits_tpm_matrix(dataset_dirs_map, subunits, match.arg(agg),
                                           datasets_to_plot, use_log2, adjust_covars = TRUE)
  MAT_adj <- res_adj$mat
  ...
  message("[TPM] AdjCovars 矩陣 CSV：", out_csv_adj)
  message("[TPM] AdjCovars 圖檔 base：", base_adj)
...
run_csn_subunits_TPM(
  dataset_dirs_map = if (exists("dataset_dirs_run")) dataset_dirs_run else NULL,
  datasets_to_plot = c("brca_cptac_2020",
                       "coad_cptac_2019",
                       "gbm_cptac_2021",
                       "luad_cptac_2020",
                       "lusc_cptac_2021",
                       "paad_cptac_2021",
                       "ucec_cptac_2020"),  # 可調整與排序
  agg = "median",
  use_log2 = FALSE,         # TRUE: 畫 log2(TPM+1)；FALSE: 直接畫 TPM
  pal = "cell",
...
## == Y 軸長 label 與輸出尺寸參數（可用 options 覆寫）==
options(
  csn_ylabel_wrap        = 40,   # 每行約 40 字元自動換行
  csn_left_margin_pts    = 160,  # 左側邊界（pt）；給長標籤空間
  csn_ylabel_extra_width = 7.5,  # 額外加到圖寬（in），避免熱圖被擠
  
  # 全域／情境化輸出尺寸（英吋）；若為 NULL 則使用函式估算的寬高
...
.save_near_only <- function(p, width, height, csv_file, meta, suffix = "_ordered", dpi = 600) {
  ...
  ggsave(paste0(near_base, ".tiff"), p, width = w_out, height = h_out, units = "in",
...
.save_both_places <- function(p, width, height, csv_file, meta,
  ...
  ggsave(paste0(collect_base, ".tiff"), p, width = w_out, height = h_out, units = "in",
         dpi = dpi_o, bg = "white", compression = "lzw")
...
.meta_save_near_v3_shortname <- function(p, width, height, csv_file, meta, suffix = "", dpi = 600) {
  ...
  ggplot2::ggsave(target, p, width = w_out, height = h_out, units = "in",
                  dpi = dpi_o, bg = "white", compression = "lzw")
}

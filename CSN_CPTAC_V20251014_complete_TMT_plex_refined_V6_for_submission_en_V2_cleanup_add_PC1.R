...
## =========================================================
## [STANDALONE] CSN PC1 Explained Variance and Loadings Export
## This section can be run independently without GSEA analysis
## =========================================================

compute_csn_pc1_variance_and_loadings <- function(dataset_dirs,
                                                  csn_subunits,
                                                  min_frac_complete = 0.75,
                                                  min_members = 5L,
                                                  pca_min_samples = 10L,
                                                  out_dir = "run_info/csn_pc1_stats") {
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  # Result collectors (combined version: COPS7A/7B -> COPS7*)
  variance_list <- list()
  loadings_list <- list()
  # Result collectors (separate version: COPS7A/7B kept separate)
  variance_list_sep <- list()
  loadings_list_sep <- list()

  for (ds_id in names(dataset_dirs)) {
    ds_dir <- dataset_dirs[[ds_id]]
    if (!dir.exists(ds_dir)) next

    log_msg("[PC1-stats] Processing: %s", ds_id)

    # Load protein matrix
    mat0 <- tryCatch(load_matrix_from_dataset_dir(ds_dir), error = function(e) NULL)
    if (is.null(mat0)) {
      log_msg("[PC1-stats] %s: Cannot load matrix, skip", ds_id)
      next
    }
    ...
    # Find present CSN subunits
    present_sub <- intersect(csn_subunits, rownames(mat0))
    if (length(present_sub) < min_members) {
      log_msg("[PC1-stats] %s: Too few subunits (%d < %d), skip", ds_id, length(present_sub), min_members)
      next
    }

    # Check sample counts with sufficient subunits
    sample_counts <- colSums(is.finite(mat0[present_sub, , drop = FALSE]))
    ok_sam <- names(sample_counts)[sample_counts >= min_members]

    if (length(ok_sam) < pca_min_samples) {
      log_msg("[PC1-stats] %s: Too few eligible samples (%d < %d), skip", ds_id, length(ok_sam), pca_min_samples)
      next
    }

    # Filter subunits: remove those with too few finite values (e.g., COPS9 all NA in PAAD)
    subunit_valid_counts <- rowSums(is.finite(mat0[present_sub, ok_sam, drop = FALSE]))
    valid_sub <- present_sub[subunit_valid_counts >= pca_min_samples]

    if (length(valid_sub) < min_members) {
      log_msg(
        "[PC1-stats] %s: Too few valid subunits after NA filtering (%d < %d), skip",
        ds_id, length(valid_sub), min_members
      )
      next
    }

    if (length(valid_sub) < length(present_sub)) {
      dropped_sub <- setdiff(present_sub, valid_sub)
      log_msg(
        "[PC1-stats] %s: Dropped subunits with insufficient data: %s",
        ds_id, paste(dropped_sub, collapse = ", ")
      )
    }

    # Z-score normalization (same method as build_csn_score)
    get_z <- function(v) {
      v <- as.numeric(v)
      mu <- mean(v[is.finite(v)], na.rm = TRUE)
      sdv <- stats::sd(v[is.finite(v)], na.rm = TRUE)
    ...
    Z_raw <- do.call(rbind, lapply(valid_sub, function(g) get_z(mat0[g, ok_sam, drop = FALSE])))
    rownames(Z_raw) <- valid_sub
    colnames(Z_raw) <- ok_sam

    # ---- VERSION 1: Without combining COPS7A/7B (separate) ----
    Z_sep <- Z_raw
    pc_sep <- tryCatch(stats::prcomp(t(Z_sep), center = TRUE, scale. = FALSE), error = function(e) NULL)
    if (!is.null(pc_sep)) {
      var_explained_sep <- 100 * pc_sep$sdev^2 / sum(pc_sep$sdev^2)
      pc1_var_pct_sep <- var_explained_sep[1]
      loadings_pc1_sep <- pc_sep$rotation[, 1]

      # Store variance result (separate version)
      variance_list_sep[[ds_id]] <- data.frame(
        dataset = ds_id,
        n_samples = length(ok_sam),
        n_subunits = nrow(Z_sep),
        subunits_used = paste(rownames(Z_sep), collapse = ";"),
        PC1_explained_variance_pct = round(pc1_var_pct_sep, 2),
        stringsAsFactors = FALSE
      )

      # Store loadings result (separate version)
      loadings_list_sep[[ds_id]] <- data.frame(
        dataset = ds_id,
        subunit = names(loadings_pc1_sep),
        PC1_loading = round(loadings_pc1_sep, 4),
        stringsAsFactors = FALSE,
        row.names = NULL
      )

      log_msg(
        "[PC1-stats-sep] %s: PC1 explained variance = %.2f%%, %d subunits (COPS7A/7B separate)",
        ds_id, pc1_var_pct_sep, nrow(Z_sep)
      )
    } else {
      log_msg("[PC1-stats-sep] %s: prcomp failed for separate version, skip", ds_id)
    }

    # ---- VERSION 2: With combining COPS7A/7B (combined) ----
    Z_comb <- Z_raw
    if (all(c("COPS7A", "COPS7B") %in% rownames(Z_comb))) {
      Z7 <- colMeans(Z_comb[c("COPS7A", "COPS7B"), , drop = FALSE], na.rm = TRUE)
      Z_comb <- rbind(Z_comb[setdiff(rownames(Z_comb), c("COPS7A", "COPS7B")), , drop = FALSE],
        "COPS7*" = Z7
      )
    }

    pc_comb <- tryCatch(stats::prcomp(t(Z_comb), center = TRUE, scale. = FALSE), error = function(e) NULL)
    if (!is.null(pc_comb)) {
      var_explained_comb <- 100 * pc_comb$sdev^2 / sum(pc_comb$sdev^2)
      pc1_var_pct_comb <- var_explained_comb[1]
      loadings_pc1_comb <- pc_comb$rotation[, 1]

      # Store variance result (combined version)
      variance_list[[ds_id]] <- data.frame(
        dataset = ds_id,
        n_samples = length(ok_sam),
        n_subunits = nrow(Z_comb),
        subunits_used = paste(rownames(Z_comb), collapse = ";"),
        PC1_explained_variance_pct = round(pc1_var_pct_comb, 2),
        stringsAsFactors = FALSE
      )

      # Store loadings result (combined version)
      loadings_list[[ds_id]] <- data.frame(
        dataset = ds_id,
        subunit = names(loadings_pc1_comb),
        PC1_loading = round(loadings_pc1_comb, 4),
        stringsAsFactors = FALSE,
        row.names = NULL
      )

      log_msg(
        "[PC1-stats-comb] %s: PC1 explained variance = %.2f%%, %d subunits (COPS7A/7B combined)",
        ds_id, pc1_var_pct_comb, nrow(Z_comb)
      )
    } else {
      log_msg("[PC1-stats-comb] %s: prcomp failed for combined version, skip", ds_id)
    }
  }
  ...
  variance_all <- NULL
  loadings_all <- NULL
  variance_all_sep <- NULL
  loadings_all_sep <- NULL

  # ---- Write combined version (COPS7A/7B -> COPS7*) ----
  if (length(variance_list) > 0) {
    variance_all <- do.call(rbind, variance_list)
    variance_file <- file.path(out_dir, "csn_pc1_explained_variance.csv")
    data.table::fwrite(variance_all, variance_file)
    log_msg("[PC1-stats] Wrote variance summary (combined): %s", variance_file)
  }

  if (length(loadings_list) > 0) {
    loadings_all <- do.call(rbind, loadings_list)
    loadings_file <- file.path(out_dir, "csn_pc1_loadings.csv")
    data.table::fwrite(loadings_all, loadings_file)
    log_msg("[PC1-stats] Wrote loadings (combined): %s", loadings_file)
  }

  # ---- Write separate version (COPS7A/7B kept separate) ----
  if (length(variance_list_sep) > 0) {
    variance_all_sep <- do.call(rbind, variance_list_sep)
    variance_file_sep <- file.path(out_dir, "csn_pc1_explained_variance_separate.csv")
    data.table::fwrite(variance_all_sep, variance_file_sep)
    log_msg("[PC1-stats] Wrote variance summary (separate): %s", variance_file_sep)
  }

  if (length(loadings_list_sep) > 0) {
    loadings_all_sep <- do.call(rbind, loadings_list_sep)
    loadings_file_sep <- file.path(out_dir, "csn_pc1_loadings_separate.csv")
    data.table::fwrite(loadings_all_sep, loadings_file_sep)
    log_msg("[PC1-stats] Wrote loadings (separate): %s", loadings_file_sep)
  }

  invisible(list(
    variance = variance_all,
    loadings = loadings_all,
    variance_separate = variance_all_sep,
    loadings_separate = loadings_all_sep
  ))
}

## Execute PC1 stats calculation (standalone, no GSEA needed)
log_msg("== Computing CSN PC1 Explained Variance and Loadings (Standalone) ==")
pc1_results <- compute_csn_pc1_variance_and_loadings(
  dataset_dirs = dataset_dirs,
  csn_subunits = csn_subunits,
  min_frac_complete = min_frac_complete,
  min_members = 5L,
  pca_min_samples = 10L,
  out_dir = file.path("run_info", "csn_pc1_stats")
)
log_msg("== Completed PC1 Stats Export ==")

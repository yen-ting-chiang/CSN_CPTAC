...
.deg_meta_make_heatmap_plot_genes <- function(csv_file,
  ...
  # x 軸位置與 gap 規則（讓欄位群組之間留白，和 proteomic GSEA meta 熱圖一致）
  # 規則：
  #   1. 在 Z_CSN_SCORE 和 Z_GPS1 之間留一小段 gap
  #   2. 在 Z_RESIDUAL_GPS1 之前再留一小段 gap
  # 這兩段 gap 其實是把 x 座標往右推一點，造成目視上的縱向留白。
...
DATASET_HEATMAP_TOP_N    <- 25   # 預設前 25 大（NES）
DATASET_HEATMAP_BOTTOM_N <- 25   # 預設前 25 小（NES）
PAN_HEATMAP_TOP_N        <- 5   # 預設前 25 大（Z）
PAN_HEATMAP_BOTTOM_N     <- 10   # 預設前 25 小（Z）
## ---- [END USER CONFIG | heatmap top/bottom limits] ----

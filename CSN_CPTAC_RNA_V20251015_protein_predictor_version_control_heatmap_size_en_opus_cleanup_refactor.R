...
##  - Output to <dataset>/csn_gsea_results_TP53/<STRATUM>/...
##    according to strata = c("ALL","TP53_mutant","TP53_wild_type")
##  - Also create pan-cancer summary and two summary tables in csn_gsea_pan_summary_TP53/
## =========================================================
...
## ===== Add yaml in the packages section (1) =====
suppressPackageStartupMessages({
  ...
  library(cowplot)
  library(matrixStats)
  library(yaml) # <-- NEW: for writing run_manifest.yml
})
  ...
  coerce_covariates_safely <- function(df) {
    ...
    for (cn in colnames(df)) {
      ...
      if (is.factor(v) || is.character(v) || is.logical(v)) {
        ...
        # To also filter out constant numeric columns, uncomment below:
        # vv <- df[[cn]]
...
## ==== Force single-threaded, disable all parallel backends (across common frameworks) ====
.force_serial_execution <- function() {
  ...
  if (requireNamespace("doParallel", quietly = TRUE)) {
    ...
    # Here we don't hard-stop unknown objects, just use DoSEQ instead.
  }
...
## Attach multiple hypothesis testing level description (updated: added meta-FDR)
cat(
  paste(
...
hi_lo_quantile <- 0.25 # 25% each for high/low
minSize <- 5
maxSize <- 1000
fgsea_eps <- 1e-10 # fgseaMultilevel() precision

min_per_group <- 8 # minimum samples for each High/Low group (skip limma if insufficient)
MAKE_PLOTS <- FALSE
...
## ---- [USER CONFIG | inline settings, no external options()] ----
...
## Write inline settings above to options for getOption(...) to read elsewhere in the script
options(csn.run_passes = RUN_PASSES)
## ---- [END USER CONFIG] ----

## ---- [USER CONFIG | heatmap top/bottom limits] ----
## Heatmap filtering threshold for non-H collections (single dataset uses NES, PAN uses Z; both based on CSN_SCORE)
...
##   "H",
##   "C1",
##   "C2", "C2:REACTOME", "C2:BIOCARTA", "C2:CGP", "C2:KEGG"  # Note: most new versions lack KEGG
##   "C3", "C3:TFT", "C3:MIR",
##   "C4", "C4:CGN", "C4:CM",
...
if (!is.null(df0) && NROW(df0) > 0) {
  df0 <- as.data.frame(df0) # avoid tibble strict subset behavior
  ...
  if (length(cat_hits) == 0 || length(name_hits) == 0) {
    log_msg("(Skipped) Required columns not found (collection or gs_name); collections list not exported.")
  } else {
    catcol <- cat_hits[1]
    subcol <- if (length(sub_hits) >= 1) sub_hits[1] else NULL
    namecol <- name_hits[1]

    ## 2) Build data frame and count unique geneset (gs_name) count
    tmp <- data.frame(
    ...
    log_msg("Exported available msigdb collections (with geneset counts) to run_info/msigdb_available_collections.csv")
  }
} else {
...
for (tok in GENESET_GROUPS_TO_RUN) {
  ...
  if (!is.null(pp$sub)) {
    ...
    # Match only at the "beginning" of subcategory string (e.g.: MIR, CP:REACTOME, GO:BP)
...
load_matrix_from_dataset_dir_protein <- function(dir) {
  ...
  if (length(keep_ids)) {
    ...
    if (length(inter) < 10) log_msg("Note: case_list intersection too small ({length(inter)}), using all sample columns instead")
  } else {
...
# For PCA cleanup: remove Inf->NA, keep rows with at least min_samples finite values; impute NA with row median
...
# Safe version audit: try original audit first; on failure use clean matrix for prcomp just for logging, don't halt execution
audit_csn_score_feasibility_safe <- function(ds_id, stratum, mat0, present_sub,
  ...
  if (is.null(X)) {
    if (exists("log_msg", mode = "function")) {
      log_msg("[CSN-audit-safe] %s | %s: insufficient available subunits or samples, skipping audit (not halting)", ds_id, stratum)
    }
  ...
  if (inherits(pc, "try-error")) {
    if (exists("log_msg", mode = "function")) {
      log_msg("[CSN-audit-safe] %s | %s: fallback prcomp still failed, skipping (not halting)", ds_id, stratum)
    }
    return(invisible(FALSE))
  }
...
._ensure_stats_names <- function(stats, gene_names, label = NULL) {
  ...
  # Don't filter yet, leave to next step for cleanup (will log)
  v
}
...
# --- utility: clean and sort GSEA stats (keep finite values, add names, dedupe, sort) ---
...
# --- utility: map pathways to stats universe and filter by size ---
...
audit_design_alignment <- function(tag, samples, mod_interest, mod_nuisance, out_dir = NULL) {
  ...
  # Extra: show first 6 rows for quick log inspection
...
## Recommend placing before run_one_stratum()
## Will append summary to run_info/covars_audit/audit_rows.csv and print one log line
...
run_fgsea_save <- function(stats, genesets, out_prefix, top_plot_n = 0, plot_title = "") {
  ...
  res <- tryCatch(
    ...
    error = function(e) {
      log_msg("    [fgseaMultilevel] failed: %s -> using fgsea()", e$message)
      suppressWarnings(
        fgsea::fgsea(
          pathways = pw_use,
          stats    = stats,
  ...
  if (top_plot_n > 0) {
    # If need to keep original plot, add here
  }
  invisible(res)
}
...
read_gsea_table <- function(out_root, subunit, group_name, stat_tag) {
  ...
  # List candidate paths based on your actual file structure (RAW/CovarAdj) and old assumptions
  candidates <- c(
...
meta_fdr_stouffer <- function(dataset_dirs,
  ...
  normalize_cols <- function(DT) {
    # Map common column names to canonical names: pathway, NES, pval, padj, size
    nm <- names(DT)
  ...
  pick_cols <- function(DT, need) {
    ...
    # Fill in missing columns to avoid rbind / column selection failure
  ...
  stouffer_df <- function(D) {
    ...
    # Unweighted Stouffer (can change to weighted)
  ...
  for (stratum in strata) {
    for (pass_label in passes) {
      for (stat_tag in stat_tags) {
        for (grp in groups) {
          # 1) Find all subunits (union of all dataset folders)
          ...
          # 2) For each subunit, collect CSV from each dataset, do Stouffer merge and write file
          for (su in subunits) {
            parts <- list()
            ...
            if (length(parts) == 0) {
              # No cohort has output for this subunit -> don't write file or report error
              next
            }
...
# Build CSN score using z-values of subunits across samples, take PC1; direction corrected to match mean z sign
build_csn_score <- function(mat0,
  ...
  if (length(keep_sam) >= 10) {
    ...
    s[keep_sam] <- sc # here alignment by sample name will always succeed
  }

  s
}

# Regress vector y (some subunit) against CSN score + batch + other covariates and get residuals
...
residualize_vector <- function(y, csn_score, batch = NULL, covars = NULL, min_n = 8L) {
  ...
  ## === NEW: first remove columns that would kill complete.cases ===
...
## ---- Auto-detect batch column & read (extended candidate columns) ----
...
.align_by_colnames <- function(vec_or_df, target_names) {
  # If provided named vector/factor/data.frame, align by colnames(mat); otherwise assume already aligned
...
.mk_batch_factor <- function(batch, sample_order, min_count_collapse = 1L) {
  ...
  b <- .sanitize_levels(b)
  # Optional: merge very small batches (default merges singletons, set 0 to disable)
...
BATCH_PIPE_POLICY <- "NA" # "NA" or "b_small": values containing '|' set to NA or merged to b_small
BATCH_MIN_PER_LEVEL <- 2 # levels below this threshold will be merged to b_small (avoid inestimable coefficients)

## ---- Batch value cleanup: handle values with '|', make names valid, merge sparse levels ----
sanitize_batch_levels <- function(x,
  ...
  # Make names valid (avoid eBayes/design matrix column name issues)
...
get_batch_factor <- function(ds_dir, sample_ids,
  ...
  # First align by sample_ids to ensure detection and returned vector have consistent length
...
## Purpose: minimal changes to seamlessly convert protein version workflow to RNA version
## Location: paste after original file "file reading and shared utilities" section, before first use of loader
## =========================


## 3) Override loader: read multiple RNA filenames (FPKM/RSEM/RPKM)
load_matrix_from_dataset_dir <- function(dir) {
  ...
  bn <- basename(fp) # "data_mrna_seq_v2_rsem.txt"
  case_candidates <- c(
...
## ---- [NEW] protein loader for predictors (does not override RNA version) ----
load_matrix_from_dataset_dir_protein <- function(dir) {
  fp <- file.path(dir, "data_protein_quantification.txt")
  if (!file.exists(fp)) stop(glue::glue("File not found: {fp}"))
...
## 4) Override impute/filter: RNA doesn't use MinProb; keep your original min_frac concept
impute_and_filter <- function(mat, min_frac = 0.75) {
  ## Only filter by "gene's non-NA proportion across samples"; typically FPKM-log has few missing values
  keep <- rowMeans(is.finite(mat)) >= min_frac
  m <- mat[keep, , drop = FALSE]

  ## If still some missing values, use light "gene-median" imputation to avoid affecting limma
  if (anyNA(m)) {
...
## 5) Other analyses (TP53 stratification, CSN score calc using RNA subunits, residual subunits,
##    limma continuous predictor, GSEA, summary/heatmap/meta-FDR) all maintain original workflow.


## ---- Batch requirement quick screen (using your logic) ----
screen_batch_need <- function(ds_dir, min_frac_complete = 0.75) {
  ...
  if (is.null(bi)) {
    log_msg("  [batch] No clear batch column found -> not adjusting")
  ...
  ## Avoid zero-variance genes causing scale() error (only affects PCA, limma F-test still uses original m)
  keep_var <- matrixStats::rowSds(m, na.rm = TRUE) > 0
  if (!all(keep_var)) log_msg("  [PCA] Removing zero-variance genes: %d", sum(!keep_var))
  ...
  if (recommend) {
    log_msg("  **Recommend adjustment**: R2 or gene proportion reached threshold (>=10%% R2 and p<0.01, or FDR<0.05 genes >=5%%)")
  } else {
    log_msg("  **No adjustment needed for now**: No obvious batch effect detected (record kept)")
  }
...
## Output prefix for this analysis
OUTPUT_PREFIX <- "mRNA_GSEA"

## Set RUN_PASSES for batch-adjusted analysis only
RUN_PASSES <<- c("CovarAdj")
...
## Old version was stopifnot(all(dir.exists(dataset_dirs))), which would halt entire batch if some directories missing.
## Changed to: list missing ones, filter dataset_dirs_run to get "actually runnable" set for this run.
...
## List of datasets to actually run this round (folder exists and contains data_protein_quantification.txt)
dataset_dirs_run <- dataset_dirs[
  dir.exists(dataset_dirs) &
    file.exists(file.path(dataset_dirs, "data_protein_quantification.txt"))
]

if (!length(dataset_dirs_run)) stop("dataset_dirs_run is empty, please verify folders and data_protein_quantification.txt exist")

log_msg("Datasets available this run (%d): %s", length(dataset_dirs_run), paste(names(dataset_dirs_run), collapse = ", "))
...
.align_to_samples <- function(x, sam, what = "covariate") {
  ...
  if (is.vector(x) || is.factor(x)) {
    # Allow length exactly equal to sam without names; otherwise prefer name alignment
    if (is.null(names(x))) {
    ...
    if (is.null(rownames(x))) {
      if (nrow(x) != length(sam)) stop(sprintf("[%s] row count mismatch: %d vs %d and no rownames to align", what, nrow(x), length(sam)))
      rownames(x) <- sam
    }
...
# Convert any object -> matrix; force set rownames; return NULL if 0 columns or row count doesn't match specified rows
...
## Convert vector safely to factor, and make NA explicit as "NA" (prefix will be added later)
...
## ---- limma t-rank (can incorporate batch / other covariates; force alignment) ----
## ==== PATCH: limma t with covariates  ====
limma_t_with_covars <- function(mat, grp2, batch = NULL, covars = NULL) {
  ...
  # Allow NA; need at least 3 finite values and variance>0
  ...
  if (!all(var_ok)) {
    ...
    if (length(drop)) log_msg("  [design] Zero/near-zero variance -> removing: %s", paste(drop, collapse = ", "))
    design <- design[, var_ok, drop = FALSE]
  }

  if (!all(c("Low", "High") %in% colnames(design))) {
    log_msg("  [design] Low/High incomplete -> skipping this limma")
    return(NULL)
  }
  ...
  if (length(removed)) log_msg("  [design] QR removing collinear: %s", paste(removed, collapse = ", "))
...
normalize_vc <- function(x) {
  # Normalize Variant_Classification: uppercase, unify separators to underscore
...
get_tp53_status <- function(ds_dir, sample_ids) {
  ...
  # Strictly use protein-altering categories; also tolerate FRAME_SHIFT_* / IN_FRAME_* prefixes
...
# Safety: if not defined earlier, add a copy here
...
summarize_tp53_counts_for_dataset <- function(ds_dir) {
  ...
  if (inherits(M, "try-error")) {
    log_msg("[TP53-audit] %s: Unable to read matrix, skipping", ds_id)
    return(NULL)
  }
  ...
  if (file.exists(mut_fp)) {
    ...
    if (!inherits(mutation_df, "try-error") &&
      ...
      if (nrow(tp53) > 0) {
        ...
        # Only count samples that are in the protein matrix
        tp53 <- tp53[tp53$Tumor_Sample_Barcode %in% sid_up, , drop = FALSE]

        # For each classification -> how many unique samples are hit
        class_counts <- tp53 |>
        ...
        # Add summary rows for Any_TP53_mutation and protein_altering (sample level)
...
## === Call (can be placed before or after main workflow; does not affect downstream analysis) ===
...
run_predictor_analyses <- function(
  ...
  csn_score_control_vec = NULL, # [One-Step] named numeric for CSN_SCORE control (if present, added to design)
  ...
  coerce_covariates_safely <- function(df) {
    ...
    for (cn in colnames(df)) {
      ...
      if (is.factor(v) || is.character(v) || is.logical(v)) {
        ...
        # Numeric stays numeric; don't force to factor
        df[[cn]] <- suppressWarnings(as.numeric(v))
        # (Optional) If you want to also remove constant numeric columns, uncomment below:
  ...
  pick_covars_df <- function(label) {
    ## NEW: Determine covariate source (fall back to df_covars0 if covars_all not in scope)
    ...
    # === NEW: First capture the drop reasons returned by select_covars_safely() (store in variable) ===
  ...
  ## [One-Step] If CSN_SCORE control is provided, add it to DF (aligned)
  ...
  ## [One-Step] Also center CSN_SCORE if present (good practice for interaction interpretation, though not strictly required for main effect)
  ...
  ## NEW: Remove all-NA columns (e.g., purity version dropped by coverage but still lingering)
  ...
  ## Align to complete rows of samples (this step is critical)
  ...
  ## --- NEW: Only add dataset/stratum prefix for RESIDUAL_* (CovarAdj); keep others unchanged ---
      ...
      try(
        {
          ...
          ## NEW: Interaction gating - TP53 needs at least 2 levels, and design must not have non-estimable TP53/interaction terms
          ...
          ## Spearman differential correlation (using uncentered predictor to calculate correlation, does not affect results)
          expr_raw_z <- as.matrix(mat0[, colnames(M_raw), drop = FALSE])
          sWT <- which(tp53_raw_fac == "WT")
          sMT <- which(tp53_raw_fac == "MT")
          min_pairs <- 10L
          if (length(sWT) >= min_pairs && length(sMT) >= min_pairs) {
            ...
            n_wt <- sum(is.finite(orig_pred_raw[sWT]))
            n_mt <- sum(is.finite(orig_pred_raw[sMT]))
            if (n_wt >= 4 && n_mt >= 4) {}
          }
        },
        silent = TRUE
      ...
      try(
        {
          ...
          ## Spearman differential correlation (uncentered predictor)
  ...
  for (grp_name in names(genesets_by_group)) {
    ...
    ## [NEW] collection-first root: <collection>/<dataset>/<stratum>/<RAW|CovarAdj>/<predictor>
    out_root_coll <- file.path(OUTPUT_PREFIX, safe_fs_name(grp_name), ds_id, basename(out_root))
    if ("RAW" %in% getOption("csn.run_passes", c("CovarAdj"))) {
...
## =========================================================
## Each stratum: produce both RAW and CovarAdj (new version)
## =========================================================
## ===== run_one_stratum:  write sample_counts, enhance batch notes (2,3,7) =====
## ===== Small function to run complete GSEA for a specified sample set (fixed version) =====
## ===== Small function to run complete GSEA for a specified sample set (fixed; CovarAdj can also include age_missing/age_z_imputed) =====
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group) {
  ...
  {
    ## [NEW] predictors source changed to protein, aligned with RNA samples
    ...
    ## 4c) RESIDUAL_<SU> (One-Step: pass raw subunit + CSN_SCORE as control)
    for (su in present_sub) {
      y <- prot_pred[su, , drop = TRUE]
      # [Refactor] Do not calculate residuals manually.
      ...
      run_predictor_analyses(
        ...
        csn_score_control_vec = csn_score_prot, # <--- NEW ARGUMENT
        is_ALL = is_ALL
      )
    }
  }
...
## Inspect currently obtained PLEX (original vs cleaned)
inspect_plex <- function(ds_dir, col = "TMT_PLEX",
  ...
  # Read clinical table and align to samples
  meta_fp <- file.path(ds_dir, "data_clinical_sample.txt")
...
select_covars_safely <- function(
  ...
  ## NEW: Re-initialize keep_cov so it has same length and column names as the now-shrunk df
  keep_cov <- rep(TRUE, ncol(df))
  names(keep_cov) <- colnames(df)
  ## /NEW
  ...
  ## NEW: Attach drop reasons for each column as attribute (caller decides whether to write CSV)
...
audit_one_dataset_sa_batch <- function(ds_dir, pipe_policy = "NA", min_per_level = 2) {
  ...
  if (has_samp && has_pat) {
    ...
    if (!is.na(sid) && !is.na(pid_samp) && !is.na(pid_pat)) {
      ...
      # AGE (raw value statistics; main workflow will z-score)
      if ("AGE" %in% names(pat)) {
        age_col <- "AGE"
  ...
  # 4) Batch check (using your existing get_batch_factor)
...
## ===== DO_AUDIT switch wraps QC/exploration (4) =====
...
if (DO_AUDIT) {
  ...
  for (ds in names(dataset_dirs)) {
    ...
    if (!dir.exists(ds_dir)) {
      log_msg("Skipping: cannot find directory %s", ds_dir)
      next
    }
  ...
  # (Not applicable for RNA version) if (dir.exists(ds_dir)) try(inspect_plex(ds_dir), silent = TRUE)
...
## Which dataset to start re-running from (can be changed)
...
## Helper: if only_strata is not set, run all by default
...
## =========================================================
## Process datasets to run in this round (complete main loop + define sample sets for each stratum)
## =========================================================
for (ds in names(dataset_dirs_run)) {
  ds_dir <- dataset_dirs_run[[ds]]
  log_msg("== Starting dataset: %s ==", ds)

  ## All datasets use batch-adjusted analysis
  ...
  ## Run three strata in order (RAW & CovarAdj are both written in run_one_stratum)
  if (.should_run("ALL")) {
    run_one_stratum(
...
## ---------- [NEW] WT vs MT delta-NES aggregation (from existing output) ----------
tp53_delta_nes_aggregate <- function(datasets_root,
  ...
  for (ds in dataset_ids) {
    ...
    for (ver in versions) {
      # Automatically detect subunits: read subdirectory names under WT/MT folders
      subunits <- unique(unlist(lapply(groups, function(g) {
        b1 <- file.path(OUTPUT_PREFIX, safe_fs_name(g), ver, ds, "TP53_wild_type")
        b2 <- file.path(OUTPUT_PREFIX, safe_fs_name(g), ver, ds, "TP53_mutant")
        c(
      ...
      for (su in subunits) {
        for (grp in groups) {
          grp_safe <- safe_fs_name(grp)
          for (st in stat_tags) {
            fp_wt <- file.path(OUTPUT_PREFIX, grp_safe, ver, ds, "TP53_wild_type", su, paste0(st, ".csv"))
            fp_mt <- file.path(OUTPUT_PREFIX, grp_safe, ver, ds, "TP53_mutant", su, paste0(st, ".csv"))
            ...
            out_dir <- file.path(OUTPUT_PREFIX, grp_safe, ver, ds, "DeltaWT_MT", su)
            dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
      ...
      message(sprintf("[delta-NES] %s/%s completed", ds, ver))
    }
  }
  invisible(TRUE)
}
...
## Condition: the for loop above is completed and all dataset/stratum/statistic results are written
## =========================================================
meta_fdr_stouffer(
  dataset_dirs = dataset_dirs_run, # <- Use the sets actually run and available this round
  strata = strata, # c("ALL","TP53_mutant","TP53_wild_type")
  stat_tags = c(
    "GSEA_limma_t_cont",
    "GSEA_limma_interaction"
  ), # limma-based analyses only
  groups = names(genesets_by_group),
  out_root = file.path(OUTPUT_PREFIX, "csn_gsea_pan_summary_TP53/meta_fdr")
)


## [NEW-1b] WT vs MT delta-NES aggregation (from already written results)
...
##   - Three versions: NoCovariate / RAW_covars / CovarAdj_covars
##   - Output: CSV + XLSX; (optional) high-resolution scatter plots tiff/png/jpg (not drawn by default)
##   - Does not depend on original GSEA loop
## =========================================================
...
# Simple residualization (without csn_score; only residualize for batch + covars)
residualize_to_covars <- function(y, batch = NULL, covars = NULL, min_n = 8L) {
  ...
  if (!is.null(covars)) {
    ...
    C <- C[sam, , drop = FALSE]
    # Safe type conversion consistent with your existing logic
    if (exists("coerce_covariates_safely", mode = "function")) C <- coerce_covariates_safely(C)
    for (cn in colnames(C)) DF[[cn]] <- C[[cn]]
  }
  yv <- suppressWarnings(as.numeric(y[sam]))
  # Remove all-NA/constant/single-level columns to avoid singular design matrix
  if (ncol(DF) > 0) {
...
# Single dataset: generate three versions of pairwise correlation
csn_pairwise_correlation_one_ds <- function(
  ...
  min_pairs = 10L,
  dpi = 600,
  width = 4.5, height = 4.5, # inches (journal friendly)
  point_size = 1.6
) {
  ...
  # Read matrix
  mat0 <- load_matrix_from_dataset_dir(ds_dir) # Your existing file reader
  present <- intersect(subunits, rownames(mat0))
  if (length(present) < 2) {
  ...
  build_covars_df <- function(ds_id, ds_dir, sample_ids) {
    pur <- get_purity_covariate(ds_id, ds_dir, sample_ids) # Existing
    sa <- get_sex_age_covariates(ds_dir, sample_ids) # Existing
    df <- data.frame(
  ...
  # Batch (if available), align with samples
  bi <- get_batch_factor(ds_dir, sam_all) # Existing
  batch <- NULL
  ...
  # Collect three version results
  all_rows <- list()

  # Plot output helper (not executed by default; controlled by MAKE_PLOTS)
  ...
  for (p in pairs) {
    ...
    ## ---- Version 2: RAW_covars (select covariates based on predictor = gx) ----
    # RAW_covars: per your new policy, directly keep sex/age/purity, no coverage/|rho| filtering
    cov_raw <- cov0
    ...
    ## ---- Version 3: CovarAdj_covars (uses sex/age/purity) ----
    cov_ba <- cov_raw
    ...
    if (sum(ok3) >= min_pairs) {
      ...
      all_rows[[length(all_rows) + 1L]] <- data.frame(
        dataset = ds_id, version = "CovarAdj_covars",
      ...
      .save_scatter(
        ...
        title = sprintf("%s | %s vs %s (CovarAdj_covars)", ds_id, gx, gy),
        out_base = file.path(ds_out, sprintf("%s_vs_%s_CovarAdj_covars", .safe_fs(gx), .safe_fs(gy)))
      )
    }
  } # end for pairs

  # Aggregate/adjust and output
  if (!length(all_rows)) {
    log_msg("[pairwise] %s: no available pairs (insufficient samples or too many NAs)", ds_id)
    return(invisible(NULL))
  }
  RES <- do.call(rbind, all_rows)
  # BH adjustment by dataset + version
...
## ---- (Optional) Execute: by default only define functions, not auto-execute ----
# run_csn_subunits_pairwise_correlations()


## =========================================================
## Direct execution: CSN subunits pairwise correlation (independent of your GSEA loop)
##  - By default no scatter plots output; to also output .tiff/.png/.jpg, change MAKE_PLOTS <- TRUE
## =========================================================
MAKE_PLOTS <- FALSE
...
##   - Make 3 versions: NoCovariate / RAW_covars / CovarAdj_covars
...
# From pairwise CSV (single dataset), organize data for a certain version's square matrix (r and padj), fill missing cells, make symmetric
# From pairwise CSV (single dataset), organize data for a certain version's square matrix (r and padj)
# Fix:
#  - Only keep subunits that actually appear in this dataset for this version (ordered by CSN_SUB_ORDER)
#  - Self-add diagonal r=1 (and don't mark black dot)
#  - Only output "lower triangle (including diagonal)" cells
...
.plot_corr_heatmap_save <- function(df_grid, title, out_base,
  ...
  ggsave(paste0(out_base, ".pdf"), p, width = width, height = height) # Vectorized
}

# Find all dataset CSVs and plot
run_csn_corrcoef_plots <- function(out_root = "RNA_CSN_subunits_correlation_coefficient") {
  if (!dir.exists(out_root)) {
    message("[corrplot] Cannot find root directory: ", out_root)
    return(invisible(FALSE))
  }
  ...
  versions <- c("NoCovariate", "RAW_covars", "CovarAdj_covars")
  for (csv in csvs) {
    ds_dir <- dirname(csv)
    ds_id <- basename(ds_dir)
    plots_dir <- file.path(ds_dir) # Place directly in dataset folder (as you requested)
    # If you want to centralize in a subfolder, change to: file.path(ds_dir, "corr_plots")

    for (ver in versions) {
      ## Pearson (keep original filename)
      ...
      ## Spearman (newly added; filename has _spearman suffix)
    ...
    message("[corrplot] Completed: ", ds_id)
  }
  invisible(TRUE)
}

## ---- Execute plotting directly (can be kept at script bottom) ----
run_csn_corrcoef_plots(out_root = "RNA_CSN_subunits_correlation_coefficient")
...
## =========================================================
## [NEW] Convert FPKM/RPKM -> TPM for CSN subunits across datasets
##   - Convert mRNA matrix (FPKM/RPKM) of each dataset to TPM (per sample independently:
##       TPM_ij = 10^6 * FPKM_ij / Sigma_k FPKM_kj; same formula for RPKM)
##   - For specified CSN subunits: aggregate samples within each dataset (default median)
##   - Output: TPM/CSN_subunits_TPM_<agg>_<metric>.csv
##   - Plot: x=dataset (can specify and order), y=subunits (fixed order), color scale=TPM or log2(TPM+1)
##   - Color palette selectable (default "Blues"); default outputs .tiff only
## =========================================================
suppressPackageStartupMessages({
  if (!requireNamespace("ggplot2", quietly = TRUE)) install.packages("ggplot2", repos = "https://cloud.r-project.org")
  if (!requireNamespace("dplyr", quietly = TRUE)) install.packages("dplyr", repos = "https://cloud.r-project.org")
...
# If defined upstream, reuse; otherwise give default order
if (!exists("CSN_SUB_ORDER", inherits = TRUE)) {
  CSN_SUB_ORDER <- c("GPS1", "COPS2", "COPS3", "COPS4", "COPS5", "COPS6", "COPS7A", "COPS7B", "COPS8", "COPS9")
}

.safe_fs <- function(x) gsub("[^A-Za-z0-9._-]+", "_", as.character(x))

# ---- FPKM/RPKM -> TPM: per-sample (column) re-scale so colSums=1e6 ----
...
# ---- Read RNA matrix (reuse your existing RNA file reader) ----
...
# ---- Build covariates (sex, age, purity) using your GSEA approach ----
.build_covars_df <- function(ds_id, ds_dir, sample_ids) {
...
# ---- Residualization (sex/age/purity) ----
.residualize_y_by_covars <- function(y, covars) {
  # If you already have residualize_to_covars(), reuse it; otherwise provide minimal implementation
  if (exists("residualize_to_covars", mode = "function")) {
    ...
    # First remove all-NA and constant/single-level columns to avoid complete.cases eliminating everything
    if (ncol(DF) > 0) {
      all_na <- vapply(DF, function(v) all(is.na(v)), logical(1))
      if (any(all_na)) DF <- DF[, !all_na, drop = FALSE]
      is_const <- vapply(DF, function(v) {
    ...
    # If no usable covariate after cleanup, use intercept-only; otherwise use 1 + .
  ...
  # To preserve scale, return adjusted vector to "original mean": residual + mean(y)
  mu <- suppressWarnings(mean(as.numeric(y[is.finite(y)]), na.rm = TRUE))
  r + ifelse(is.finite(mu), mu, 0)
}

# ---- Build subunits x datasets TPM summary matrix (default: median within each dataset) ----
build_csn_subunits_tpm_matrix <- function(dataset_dirs_map,
  ...
  if (is.null(dataset_dirs_map)) {
    if (exists("dataset_dirs_run")) {
      ...
      stop("Cannot find dataset directory mapping (dataset_dirs_run / dataset_dirs)")
    }
  }
  ...
  if (!length(keep)) stop("datasets_to_plot and available datasets have no intersection")

  # Collect summary values for each dataset's subunits
  out_list <- list()
  for (ds in keep) {
    ds_dir <- dataset_dirs_map[[ds]]
    if (!dir.exists(ds_dir)) next
    mat0 <- .read_rna_mat(ds_dir)
    # For brca_cptac_2020 and gbm_cptac_2021, COPS9 is named MYEOV2 in RNA files
    # Rename MYEOV2 to COPS9 so the data is correctly mapped
    ...
    if (isTRUE(adjust_covars)) {
      ...
      cov_df <- .build_covars_df(ds, ds_dir, sam_all)
      # Residualize per gene (sex/age/purity), then summarize
      adj_mat <- sub_mat
...
.get_palette <- function(name, n = 256) {
  ...
  ## Cell journal green-yellow palette: low=light yellow, high=moderate green (avoid too dark near black)
  if (nm %in% c("cell", "cell_gy", "cellgy", "greenyellow")) {
    return(colorRampPalette(c("#FFF7BC", "#B8E186", "#2CA25F"))(n))
  }

  if (nm %in% c("viridis", "magma", "plasma", "inferno", "cividis") &&
  ...
  if (requireNamespace("RColorBrewer", quietly = TRUE)) {
    ...
    return(colorRampPalette(RColorBrewer::brewer.pal(9, bnm))(n))
  }

  ## fallback: blue palette
  colorRampPalette(c("#f7fbff", "#6baed6", "#08306b"))(n)
}

# ---- Draw heatmap and output ----
plot_csn_subunits_tpm_heatmap <- function(MAT, metric = "log2TPM",
                                          ds_order = colnames(MAT),
                                          pal = "cell",
                                          formats = c("tiff"),
                                          out_dir = "TPM",
  ...
  df <- as.data.frame(MAT) %>%
    tibble::rownames_to_column("subunit") %>%
    tidyr::pivot_longer(-subunit, names_to = "dataset", values_to = "value")
  df$subunit <- factor(df$subunit, levels = CSN_SUB_ORDER)
  ds_order <- intersect(ds_order, unique(df$dataset))
  df$dataset <- factor(df$dataset, levels = ds_order)
  df$label <- formatC(df$value, format = "f", digits = label_digits)

  p <- ggplot(df, aes(x = dataset, y = subunit, fill = value)) +
    geom_tile(color = "white", size = 0.3) +
    geom_text(aes(label = label), color = label_color, size = label_size) +
    scale_y_discrete(limits = rev(levels(df$subunit))) + # Display CSN_SUB_ORDER from top to bottom
    scale_fill_gradientn(colours = .get_palette(pal), name = metric) +
  ...
  base <- file.path(out_dir, sprintf(
    "CSN_subunits_%s_heatmap_%s%s",
    metric, .safe_fs(pal),
    ifelse(nchar(name_suffix), paste0("_", .safe_fs(name_suffix)), "")
  ))
  if ("tiff" %in% formats) ggplot2::ggsave(paste0(base, ".tiff"), p, width = width, height = height, dpi = dpi, compression = "lzw")
  if ("png" %in% formats) ggplot2::ggsave(paste0(base, ".png"), p, width = width, height = height, dpi = dpi)
  if ("jpg" %in% formats) ggplot2::ggsave(paste0(base, ".jpg"), p, width = width, height = height, dpi = dpi)
  if ("pdf" %in% formats) ggplot2::ggsave(paste0(base, ".pdf"), p, width = width, height = height)
  invisible(base)
}

# ---- One-click: build TPM matrix + save CSV + plot ----
run_csn_subunits_TPM <- function(dataset_dirs_map = NULL,
                                 subunits = CSN_SUB_ORDER,
                                 datasets_to_plot = NULL, # Which datasets to plot and their order (string vector)
                                 agg = c("median", "mean"),
                                 use_log2 = TRUE,
                                 pal = "cell",
                                 label_digits = 1,
                                 label_color = "black",
                                 label_size = 3,
                                 formats = c("tiff")) { # Default outputs .tiff only
  res <- build_csn_subunits_tpm_matrix(dataset_dirs_map, subunits, match.arg(agg), datasets_to_plot, use_log2)
  MAT <- res$mat
  metric <- res$metric
  ds_order <- if (!is.null(datasets_to_plot)) datasets_to_plot else res$order_ds

  dir.create("TPM", showWarnings = FALSE)

  ## ---- RAW version ----
  res_raw <- build_csn_subunits_tpm_matrix(dataset_dirs_map, subunits, match.arg(agg),
    datasets_to_plot, use_log2,
    adjust_covars = FALSE
  )
  MAT_raw <- res_raw$mat
  metric <- res_raw$metric
  ds_order <- if (!is.null(datasets_to_plot)) datasets_to_plot else res_raw$order_ds

  out_csv_raw <- file.path("TPM", sprintf("CSN_subunits_TPM_%s_%s__RAW.csv", match.arg(agg), metric))
  readr::write_csv(as.data.frame(MAT_raw) %>% tibble::rownames_to_column("subunit"), out_csv_raw)

  base_raw <- plot_csn_subunits_tpm_heatmap(MAT_raw,
    metric = ifelse(metric == "TPM", "TPM", "log2(TPM + 1)"),
    ds_order = ds_order, pal = pal, formats = formats, out_dir = "TPM",
    label_digits = label_digits, label_color = label_color, label_size = label_size,
    name_suffix = "RAW"
  )
  message("[TPM] RAW matrix CSV: ", out_csv_raw)
  message("[TPM] RAW plot base: ", base_raw)

  ## ---- AdjCovars (sex/age/purity) version ----
  res_adj <- build_csn_subunits_tpm_matrix(dataset_dirs_map, subunits, match.arg(agg),
    datasets_to_plot, use_log2,
    adjust_covars = TRUE
  )
  MAT_adj <- res_adj$mat
  out_csv_adj <- file.path("TPM", sprintf("CSN_subunits_TPM_%s_%s__AdjCovars.csv", match.arg(agg), metric))
  readr::write_csv(as.data.frame(MAT_adj) %>% tibble::rownames_to_column("subunit"), out_csv_adj)

  base_adj <- plot_csn_subunits_tpm_heatmap(MAT_adj,
    metric = ifelse(metric == "TPM", "TPM", "log2(TPM + 1)"),
    ds_order = ds_order, pal = pal, formats = formats, out_dir = "TPM",
    label_digits = label_digits, label_color = label_color, label_size = label_size,
    name_suffix = "AdjCovars"
  )
  message("[TPM] AdjCovars matrix CSV: ", out_csv_adj)
  message("[TPM] AdjCovars plot base: ", base_adj)

  invisible(list(
    csv_raw = out_csv_raw, plot_base_raw = base_raw, matrix_raw = MAT_raw,
    csv_adj = out_csv_adj, plot_base_adj = base_adj, matrix_adj = MAT_adj
  ))
}

## ---- Usage example (enable/disable as needed) ----
run_csn_subunits_TPM(
  ...
  datasets_to_plot = c(
    ...
    "paad_cptac_2021",
    "ucec_cptac_2020"
  ), # Can adjust and order
  agg = "median",
  use_log2 = FALSE, # TRUE: plot log2(TPM+1); FALSE: plot TPM directly
  pal = "cell",
  formats = c("tiff") # Can change to c("tiff","png","jpg","pdf")
)
## =========================================================


## =========================================================
## Batch plot single dataset/stratum GSEA Summary CSV to heatmap (heatmap_gsea_ggplot_V4.R)
## - Compatible with RAW and CovarAdj (including limma_t, interaction)
## - Fully reuses heatmap_gsea_ggplot.txt file reading, long table conversion, coloring and dot marking rules
## - Produces two sets of output: in-place output & aggregated output single_dataset_GSEA_heatmap/<dataset>/
## =========================================================
...
## == Y-axis long label and output size parameters (can be overridden with options) ==
options(
  csn_ylabel_wrap = 40, # Auto-wrap at about 40 characters per line
  csn_left_margin_pts = 160, # Left margin (pt); give space for long labels
  csn_ylabel_extra_width = 7.5, # Extra width added to figure (in), avoid squeezing heatmap

  # Global/contextual output size (inches); if NULL, use function-estimated width/height
  csn_fig_width_in = NULL,
  csn_fig_height_in = NULL,
  csn_dataset_fig_width_in = NULL,
  csn_dataset_fig_height_in = NULL,
  csn_meta_fig_width_in = NULL,
  csn_meta_fig_height_in = NULL,

  # Global DPI (default 600)
  csn_fig_dpi = 600
)


## ===== Cell style: positive orange, negative green (multiple options available) =====
...
# Use options(csn_interaction_palette = "cell_og3") to select palette; default is cell_og2
...
# Same rules as .make_heatmap_plot_with_yorder, but allows specifying orange/green colors
.make_heatmap_plot_with_yorder_colored <- function(csv_file, y_order, palette = .get_interaction_palette()) {
  ...
  # Y-axis uses ALL's order; only draw ALL's geneset and follow its order exactly
  ...
  # Gap rules and positions (same as your original)
  gap <- 0.4
...
# Target X-axis order (same as your original script)
.pred_order_all <- c(
...
.make_heatmap_plot <- function(csv_file) {
  ...
  # Auto-detect pathway column (same as your original script)
  path_candidates <- c("pathway", "Pathway", "term", "Term", "gs_name", "NAME", "set", "Set")
  path_col <- intersect(path_candidates, names(df_raw))[1]
  if (is.na(path_col)) stop("Cannot find pathway column: ", paste(path_candidates, collapse = ", "))

  # Long/wide table detection + convert to long table (same as original)
  has_long <- all(c("predictor", "NES", "padj") %in% names(df_raw))
  if (has_long) {
    df_long <- df_raw %>%
  ...
  present_preds <- intersect(.pred_order_all, unique(df_long$predictor))
  if (!length(present_preds)) stop("No specified predictors in data.")
  df_long <- df_long %>% filter(predictor %in% present_preds)
  ## [NEW] Non-H collection: filter top/bottom N rows by CSN_SCORE NES
  ...
  # Y-axis sort: prefer NES_CSN_SCORE descending; otherwise use row mean
  ...
  # X-axis unequal spacing positions and two gaps
  # Rule adjustment: even if "no NES_COPS9", still keep second gap if NES_RESIDUAL_GPS1 exists
  gap <- 0.4
  needs_gap1 <- all(c("NES_CSN_SCORE", "NES_GPS1") %in% present_preds)
  ...
  p <- ggplot(df_long, aes(x = xpos, y = pathway, fill = NES)) +
    ...
    theme(
      ...
      legend.position = "right",
      plot.margin = margin(6, 12, 6, getOption("csn_left_margin_pts", 160)),
      plot.background = element_rect(fill = "white", colour = NA),
      panel.background = element_rect(fill = "white", colour = NA)
    ) +
    coord_cartesian(clip = "off")

  # Size: adjust based on pathway count
  n_path <- nlevels(df_long$pathway)
...
# Read CSV and compute pathway order based on .make_heatmap_plot rules (returns vector)
.compute_pathway_order_from_csv <- function(csv_file) {
  ...
  ## -- Key: use ALL layer's "top 25 up + bottom 25 down (by CSN_SCORE)" as the only geneset list -- ##
  .coll_tok <- sub("^Summary_(.+?)_GSEA.*$", "\\1", basename(csv_file))
  if ("NES_CSN_SCORE" %in% present_preds) {
    csn_tbl <- df_long %>%
      dplyr::filter(.data$predictor == "NES_CSN_SCORE") %>%
    ...
    # Non-H: only keep top/bottom; H: 50 items stay as is, equivalent to top/bottom=all
  ...
  # Order by NES_CSN_SCORE descending (or row mean) to give "order containing only keep"
  if ("NES_CSN_SCORE" %in% present_preds) {
    nes_csn <- df_long %>%
      dplyr::filter(.data$predictor == "NES_CSN_SCORE") %>%
      dplyr::select(.data$pathway, .data$NES) %>%
...
# Plot with given y_order (pathway levels) (rules fully follow .make_heatmap_plot)
.make_heatmap_plot_with_yorder <- function(csv_file, y_order) {
  ...
  # Use ALL's order; only draw ALL's geneset and follow its order exactly
  df_long <- df_long %>% dplyr::filter(.data$pathway %in% y_order)
  y_levels <- y_order[y_order %in% df_long$pathway]
  if (!length(y_levels)) stop("Ordered version: no intersection with ALL's pathway order.")
  df_long <- df_long %>% dplyr::mutate(pathway = factor(.data$pathway, levels = rev(y_levels)))
  ...
  # Gap rules, coloring, and dot marking - same as original
  gap <- 0.4
  needs_gap1 <- all(c("NES_CSN_SCORE", "NES_GPS1") %in% present_preds)
...
.save_near_only <- function(p, width, height, csv_file, meta, suffix = "_ordered", dpi = 600) {
  ...
  # Size/DPI override (single dataset heatmap)
  w_out <- getOption("csn_dataset_fig_width_in", getOption("csn_fig_width_in", width))
  h_out <- getOption("csn_dataset_fig_height_in", getOption("csn_fig_height_in", height))
  dpi_o <- getOption("csn_fig_dpi", dpi)

  ggsave(paste0(near_base, ".tiff"), p,
    width = w_out, height = h_out, units = "in",
    dpi = dpi_o, bg = "white", compression = "lzw"
  )
  # ggsave(paste0(near_base, ".png"),  p, width = w_out, height = h_out, units = "in",
  #       dpi = dpi_o, bg = "white")
  # ggsave(paste0(near_base, ".jpg"),  p, width = w_out, height = h_out, units = "in",
  #       dpi = dpi_o, bg = "white", quality = 100)
  # ggsave(paste0(near_base, ".pdf"),  p, width = w_out, height = h_out, units = "in",
  #       bg = "white")
}
...
.parse_meta_from_path <- function(csv_file) {
  ...
  # Variant (RAW / CovarAdj): grab from any level
  vidx <- which(parts_lc %in% c("raw", "CovarAdj"))
  ...
  # Fallback: if previous step didn't catch it, search any level once
  if (is.na(stratum) || !nzchar(stratum)) {
    hit <- which(parts_lc %in% c("all", "tp53_mutant", "tp53_wild_type"))
    if (length(hit)) stratum <- parts[hit[1]]
  }

  # Dataset: if path contains csn_gsea_results_TP53, take the level above it; otherwise let caller fallback
...
.save_both_places <- function(p, width, height, csv_file, meta,
  ...
  # ggsave(paste0(collect_base, ".png"),  p, width = w_out, height = h_out, units = "in",
  #       dpi = dpi_o, bg = "white")
  # ggsave(paste0(collect_base, ".jpg"),  p, width = w_out, height = h_out, units = "in",
  #       dpi = dpi_o, bg = "white", quality = 100)
  # ggsave(paste0(collect_base, ".pdf"),  p, width = w_out, height = h_out, units = "in",
  #       bg = "white")
}


# Scan a single dataset root directory to get the specified Summary CSV list
.find_target_csvs_for_dataset <- function(ds_dir) {
  # Search for Summary files matching the name
  files <- list.files(
    ds_dir,
  ...
  # Still require RAW or CovarAdj in the path (accepts both / and \\)
  sep_pat <- "[/\\\\]" # Match both / and \\
  keep <- grepl(paste0(sep_pat, "raw", sep_pat), files, ignore.case = TRUE) |
    grepl(paste0(sep_pat, "CovarAdj", sep_pat), files, ignore.case = TRUE)
  files <- files[keep]


  ## ===== This is the block you need to add (before return) =====
...
run_gsea_heatmaps_for_all_datasets <- function(dataset_dirs_map = NULL,
  ...
  for (ds in names(dataset_dirs_map)) {
    ds_dir <- dataset_dirs_map[[ds]]
    scan_dir <- OUTPUT_PREFIX
    csvs <- .find_target_csvs_for_dataset(scan_dir)
    ## Filter by dataset name (also require path contains /summary/)
    ...
    # If groups are specified (e.g., only run H), further limit paths to these groups
    ...
    ## [NEW] Filter collections to plot (single dataset heatmap)
    ...
    if (!length(csvs)) {
      message("[heatmap] ", ds, ": No matching Summary CSV found, skipping.")
      next
    }
    for (csv in csvs) {
      meta <- .parse_meta_from_path(csv)
      # If dataset name cannot be inferred from path, fall back to the key of dataset_dirs_map
      if (!length(meta$dataset) || is.na(meta$dataset) || meta$dataset == "") {
        meta$dataset <- ds
      }

      # If it is interaction but stratum is not ALL, skip (just in case)
      if (grepl("GSEA_limma_interaction", basename(csv), fixed = TRUE) &&
        !identical(meta$stratum, "ALL")) {
        next
      }

      ## === Fix: Apply green-orange Cell color scheme to general limma_interaction ===
      ...
      ## === NEW: _ordered version for interaction (based on limma_t_cont ALL of same dataset + same variant) ===
      # Only process interaction at ALL stratum (your data structure only has interaction at ALL)
      bn_lc <- tolower(basename(csv))
      if (grepl("gsea_limma_interaction", bn_lc, fixed = TRUE) &&
        identical(meta$stratum, "ALL")) {
        # Target limma_t_cont ALL path: change the folder from GSEA_limma_interaction to GSEA_limma_t_cont,
        # and change limma_interaction to limma_t_cont in the filename
        ...
        if (file.exists(cont_csv)) {
          ...
          if (!inherits(y_order, "try-error")) {
            ...
            if (!inherits(h2, "try-error")) {
              ...
              message("[heatmap][interaction-ordered] Completed (based on limma_t_cont ALL): ", csv)
            } else {
              message("[heatmap][interaction-ordered] Chart creation failed, skipping: ", csv, " | ", as.character(h2))
            }
          } else {
            message("[heatmap][interaction-ordered] Failed to get limma_t_cont ALL order, skipping: ", cont_csv, " | ", as.character(y_order))
          }
        } else {
          message("[heatmap][interaction-ordered] Cannot find corresponding limma_t_cont ALL, skipping: ", cont_csv)
        }
      }

      ## === NEW: ordered version for TP53_mutant / TP53_wild_type (based on ALL's y-axis order) ===
      # Only for limma_t_cont and spearman methods
      if (tolower(meta$stratum) %in% c("tp53_mutant", "tp53_wild_type")) {
        bn_lc <- tolower(basename(csv))
        is_cont <- grepl("gsea_limma_t_cont", bn_lc, fixed = TRUE)
        if (is_cont) {
          # Construct the corresponding ALL path based on "same dataset + same variant + same collection (if any)":
          # Directly change the stratum directory (ALL/TP53_mutant/TP53_wild_type) in the current path to ALL
          parts <- strsplit(normalizePath(csv, winslash = "/"), "/")[[1]]
          parts_lc <- tolower(parts)
          sidx <- which(parts_lc %in% c("all", "tp53_mutant", "tp53_wild_type"))
          if (length(sidx)) {
            parts2 <- parts
            parts2[sidx[length(sidx)]] <- "ALL" # Change the actual stratum directory to ALL
            all_csv <- paste(parts2, collapse = "/")
            if (file.exists(all_csv)) {
              # Get the pathway order from ALL
              y_order <- try(.compute_pathway_order_from_csv(all_csv), silent = TRUE)
              if (!inherits(y_order, "try-error")) {
                # Redraw current CSV based on ALL order
                h2 <- try(.make_heatmap_plot_with_yorder(csv, y_order), silent = TRUE)
                if (!inherits(h2, "try-error")) {
                  .save_near_only(h2$plot, h2$width, h2$height, csv, meta, suffix = "_ordered")
                  message("[heatmap][ordered] Completed (based on ALL order): ", csv)
                } else {
                  message("[heatmap][ordered] Chart creation failed, skipping: ", csv, " | ", as.character(h2))
                }
              } else {
                message("[heatmap][ordered] Failed to get ALL order, skipping: ", all_csv, " | ", as.character(y_order))
              }
            } else {
              message("[heatmap][ordered] Cannot find corresponding ALL file, skipping: ", all_csv)
            }
          }
        }
      }
      # ---- Clean up connections and memory after processing each file (avoid Windows "Too many open files") ----
...
options(
  csn_dataset_fig_width_in = 9,
  csn_dataset_fig_height_in = 14,
  csn_meta_fig_width_in = 10,
  csn_meta_fig_height_in = 12,
  csn_fig_dpi = 600,
  csn_ylabel_wrap = 40,
  csn_left_margin_pts = 0,
...
run_gsea_heatmaps_for_all_datasets(
  dataset_dirs_map = if (exists("dataset_dirs_run")) dataset_dirs_run else NULL,
  collect_root = file.path(OUTPUT_PREFIX, "single_dataset_GSEA_heatmap")
)


## =========================================================
## [NEW] Summarize meta-FDR (Stouffer to BH) across subunits
...
## ---- One-click execute (do not re-run GSEA; only read existing *_meta_fdr.csv and summarize) ----
...
## Execute once directly (can be run repeatedly; will overwrite same-name csv)
...
##   - Input:
##     csn_gsea_pan_summary_TP53/meta_fdr/<STRATUM>/<RAW|CovarAdj>/<SUBUNIT>/<GROUP>/GSEA_limma_interaction_meta_fdr.csv
##   - Output:
##     csn_gsea_pan_summary_TP53/meta_fdr/summary/<STRATUM>/<RAW|CovarAdj>/<GROUP>/GSEA_limma_interaction_meta_fdr/
...
##   - Does not re-run GSEA; reuses summarize_meta_fdr_across_subunits() and related tools you added previously.
## =========================================================
...
## ---- Execute once directly (can be run repeatedly; will only overwrite output files, not re-run GSEA) ----
...
##   - Output: same folder as input; filename prefix = heatmap_<STRATUM>_<VERSION>_
##   - ordered version:
##       a) (ALL) interaction  -> based on y-order of limma_t_cont_meta_fdr of "same version" ALL
##       b) (TP53_mutant / TP53_wild_type) t_cont  -> based on y-order of limma_t_cont_meta_fdr of "same version" ALL
## =========================================================

## ===== Safely load old utility functions (only create if not defined) =====

if (!exists(".read_csv_safe", mode = "function")) {
  .read_csv_safe <- function(path) {
    p <- path
    # Windows: if original string does not exist, try / -> \\
    if (!file.exists(p) && .Platform$OS.type == "windows") {
      p2 <- gsub("/", "\\\\", p, fixed = TRUE)
      if (file.exists(p2)) p <- p2
    }
...
if (!exists(".pred_order_all", mode = "any")) {
  # Target X-axis order (reuse from your original script)
  .pred_order_all <- c(
    "NES_CSN_SCORE", "NES_GPS1",
    "NES_COPS2", "NES_COPS3", "NES_COPS4", "NES_COPS5", "NES_COPS6",
    "NES_COPS7A", "NES_COPS7B", "NES_COPS8", "NES_COPS9",
...
# Z / padj_meta predictor order (reuse your original NES_* order)
...
.meta_read_summary_long <- function(csv_file) {
  ...
  if (is.na(path_col)) stop("Cannot find pathway column in: ", csv_file)

  z_cols <- grep("^Z_", names(df), value = TRUE)
  padj_cols <- grep("^padj_meta_", names(df), value = TRUE)
  if (!length(z_cols)) stop("Cannot find Z_* columns in: ", csv_file)
...
# Calculate y-axis order using meta file (Z): prioritize Z_CSN_SCORE descending, otherwise use row average Z
.meta_compute_y_order <- function(csv_file) {
  ...
  ## -- KEY: Use ALL stratum's "top 25 + bottom 25 (by Z_CSN_SCORE)" as the unique geneset list -- ##
  .coll_tok <- sub("^Summary_(.+?)_GSEA.*$", "\\1", basename(csv_file))
  ...
  # Sort by Z_CSN_SCORE descending (or row average Z) to get "order containing only keep"
  if ("Z_CSN_SCORE" %in% present) {
    ord <- df_long %>%
      dplyr::filter(.data$predictor == "Z_CSN_SCORE") %>%
      dplyr::select(.data$pathway, .data$Z) %>%
...
# Build meta-FDR heatmap (Z for color scale; black dot for padj_meta<0.05)
.meta_make_heatmap_plot <- function(csv_file, y_order = NULL,
                                    palette = c(low = "#053061", mid = "#FFFFFF", high = "#67001F")) {
  df_long <- .meta_read_summary_long(csv_file)

  present <- intersect(.pred_order_meta, unique(df_long$predictor))
  if (!length(present)) stop("No specified predictors in data: ", csv_file)
  df_long <- dplyr::filter(df_long, .data$predictor %in% present)
  ...
  ##      only precisely specify 50 genesets using y_order later.
  .coll_tok <- sub("^Summary_(.+?)_GSEA.*$", "\\1", basename(csv_file))
  if (is.null(y_order)) {
  ...
  # y-order: use external if provided, otherwise by Z_CSN_SCORE or average Z
  if (is.null(y_order)) {
    y_order <- .meta_compute_y_order(csv_file)
  }
  # Use ALL's order; only draw ALL's geneset and follow its order exactly (same as single dataset ordered)
  df_long <- df_long %>% dplyr::filter(.data$pathway %in% y_order)
  y_levels <- y_order[y_order %in% df_long$pathway]
...
# Parse meta_fdr/summary path to get version(=RAW/CovarAdj) and stratum
.meta_parse_summary_path <- function(csv_file) {
  parts <- strsplit(normalizePath(csv_file, winslash = "/"), "/")[[1]]
  parts_lc <- tolower(parts)
  ver_idx <- which(parts_lc %in% c("raw", "CovarAdj"))
...
# Save to "same folder as input", filename prefixed with: heatmap_<STRATUM>_<VERSION>_
.meta_save_near <- function(p, width, height, csv_file, meta, suffix = "", dpi = 600) {
  ...
  # -- Shorten filename: only take group and method token to avoid Windows path truncation --
...
# ---- v2: Force short filename saving to avoid Windows path length truncation ----
...
.meta_save_near_v3_shortname <- function(p, width, height, csv_file, meta, suffix = "", dpi = 600) {
  ...
  # Size/DPI override (meta-FDR heatmap)
  w_out <- getOption("csn_meta_fig_width_in", getOption("csn_fig_width_in", width))
  h_out <- getOption("csn_meta_fig_height_in", getOption("csn_fig_height_in", height))
  dpi_o <- getOption("csn_fig_dpi", dpi)
...
# Find four types of target CSV (only get group=H's ALL / MUT / WT, and filename must match exactly)
.meta_find_target_csvs <- function(root = "csn_gsea_pan_summary_TP53/meta_fdr/summary") {
  ...
  # Still require path to contain RAW/CovarAdj
  keep <- grepl("(/|\\\\)(RAW)(/|\\\\)", files, ignore.case = TRUE) |
    grepl("(/|\\\\)(CovarAdj)(/|\\\\)", files, ignore.case = TRUE)
  files <- files[keep]
  if (!length(files)) {
    return(files)
  }
...
# If not yet defined, use old version of scan function
...
# If not yet defined, use old version of .parse_meta_from_path()
...
# Main program: plot for found meta_fdr Summary files, and generate ordered version (based on ALL/t_cont)
run_meta_fdr_heatmaps <- function(root = file.path(OUTPUT_PREFIX, "csn_gsea_pan_summary_TP53/meta_fdr/summary")) {
  ...
  for (csv in csvs) {
    ...
    ## If it is interaction, use green-orange Cell palette
    pal_cell <- NULL
    if (is_interaction) {
    ...
    if (inherits(h, "try-error")) {
      message("[meta-heatmap] Chart creation failed: ", csv, " | ", as.character(h))
      next
    }
    .meta_save_near_v3_shortname(h$plot, h$width, h$height, csv, meta, suffix = "")


    # ----- Check if ordered version should be drawn -----
    # Case A: ALL's interaction -> based on order of limma_t_cont_meta_fdr of same version ALL
    ...
    if (is_interaction_all && identical(tolower(meta$stratum), "all")) {
      ...
      if (file.exists(cont_csv)) {
        ...
        if (!inherits(yo_full, "try-error")) {
          # Use ALL/t_cont's CSV to directly determine "those 50" and their order
          ...
          if (!identical(toupper(.coll_tok2), "H")) {
            ...
            keep50 <- yo_full # Hallmark itself is usually 50
          }
          # Extract those 50 according to ALL/t_cont's order
          ...
          if (!inherits(h2, "try-error")) {
            ...
            message("[meta-heatmap][ordered] Chart creation failed: ", csv, " | ", as.character(h2))
          }
        } else {
    ...
    # Case B: TP53_mutant / TP53_wild_type's t_cont -> based on same version ALL's t_cont
      ...
      if (length(st_idx)) {
        ...
        if (file.exists(all_csv)) {
          ...
          if (!inherits(yo_full, "try-error")) {
            ...
            # Extract those 50 according to ALL/t_cont's order
            y_levels <- yo_full[yo_full %in% keep50]
            h3 <- try(.meta_make_heatmap_plot(csv, y_order = y_levels), silent = TRUE) # t_cont keeps blue-red
            if (!inherits(h3, "try-error")) {
              .meta_save_near_v3_shortname(h3$plot, h3$width, h3$height, csv, meta, suffix = "_ordered")
            } else {
              message("[meta-heatmap][ordered] Chart creation failed: ", csv, " | ", as.character(h3))
            }
          } else {
            message("[meta-heatmap][ordered] Failed to get ALL/t_cont y-order: ", all_csv, " | ", as.character(yo_full))
          }
        } else {
...
options(
  csn_dataset_fig_width_in = 9,
  csn_dataset_fig_height_in = 14,
  csn_meta_fig_width_in = 15,
  csn_meta_fig_height_in = 11,
  csn_fig_dpi = 600,
  csn_ylabel_wrap = 40,
  csn_left_margin_pts = 0,
  csn_ylabel_extra_width = 7.5
...
run_meta_fdr_heatmaps(
  root = file.path(OUTPUT_PREFIX, "csn_gsea_pan_summary_TP53/meta_fdr/summary")
)

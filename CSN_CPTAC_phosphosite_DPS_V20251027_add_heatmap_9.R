...
plot_dps_meta_heatmap_for_pathways <- function(
    ...
    sig_dot_size   = 0.6,      ## [NEW] 顯著性點大小（三個 stratum 同步套用）
    out_formats = c("tiff"),    ## 也可指定 c("tiff","png","pdf","jpg")
    fig_width_in  = NULL,       ## [NEW] 指定輸出寬度（英吋）；NULL = 依資料自動計算
    fig_height_in = NULL        ## [NEW] 指定輸出高度（英吋）；NULL = 依資料自動計算
){
  if (!requireNamespace("data.table", quietly = TRUE)) stop("請先安裝 data.table")
  if (!requireNamespace("ggplot2",    quietly = TRUE)) stop("請先安裝 ggplot2")
  if (!requireNamespace("dplyr",      quietly = TRUE)) stop("請先安裝 dplyr")
  ...
  ## y 軸目標順序：
  ##   - 我們要「Z_CSN_SCORE 越大越上面」
  ##   - ggplot 畫 y=因子時，levels 的第1個會在最下方，最後一個會在最上方
  ##   - 所以我們要把最負的放最前面，最正的放最後面
  ##   - sel_neg 是從最負開始往上、sel_pos 是從最大正開始往下排的 rev()
  ##   → 先把 sel_neg 放前面，再把 rev(sel_pos) 放後面
  site_order_raw <- c(sel_neg, rev(sel_pos))
  
  ## 去除重複（有些 site 可能同時進入 sel_neg 和 sel_pos）
  ...
  .make_stratum_plot <- function(prep_obj, stratum_label){
    ...
    ggplot2::ggplot(
      ...
      ggplot2::geom_point(
        ...
        inherit.aes = FALSE,
        shape       = 16,     # 實心圓
        size        = sig_dot_size,    # [NEW] 可調整點大小 (預設 0.6)
        colour      = "black"
      ) +

...
PAN_HEATMAP_TOP_N        <- 40   # 預設前 25 大（Z）
...
## 設定要保留的 pathway 前綴（例如只畫 KINASE-PSP* 與 PATH-NP*）。
## - 設為 NULL 代表不過濾（預設）。
## - 例如：options(HEATMAP_PATHWAY_PREFIX = c("KINASE-PSP","PATH-NP"))
...
## 控制熱圖中「顯著性圓點」大小（單一 dataset 與 PAN/meta 可分別設定）
...
get_batch_factor_phospho <- function(ds_dir, sample_ids,
  ...
  if (file.exists(meta_fp)) {
    ...
    if (length(id_cols)) {
      meta <- dplyr::rename(meta, SAMPLE_ID = !!id_cols[1])
      meta$SAMPLE_ID <- as.character(meta$SAMPLE_ID)
      ## 與 protein 版一致：用 match 對齊並保留 sample_ids 的順序與長度
      meta <- meta[match(sample_ids, meta$SAMPLE_ID), , drop = FALSE]
      rownames(meta) <- sample_ids
      det <- detect_batch_column(meta,
                                 pipe_policy   = pipe_policy,
  ...
  if (!file.exists(tmt_fp)) return(NULL)
  
  tmt <- suppressMessages(readr::read_csv(tmt_fp, show_col_types = FALSE)) |> as.data.frame()
  cn <- names(tmt); cn_trim <- trimws(cn); names(tmt) <- cn_trim
  run_hits <- grep("^Run\\s*Metadata\\s*ID$", cn_trim, ignore.case = TRUE, value = TRUE)
  tmt_cols <- grep("^tmt_",               cn_trim, ignore.case = TRUE, value = TRUE)
  if (!length(run_hits) || !length(tmt_cols)) return(NULL)
  ...
  if (!length(plex_by_sample)) return(NULL)
  v <- rep(NA_character_, length(sample_ids)); names(v) <- sample_ids
  mm <- intersect(names(plex_by_sample), sample_ids)
  if (length(mm)) v[mm] <- unlist(plex_by_sample[mm], use.names = FALSE)
...
screen_batch_need <- function(ds_dir, min_frac_complete = 0.75){
  ...
  ## 安全 PCA：先清 NA/Inf，保留至少 10 個有效樣本與 ≥5 基因；否則略過 PCA
  X <- .clean_for_pca(m, min_samples = 10L, min_genes = 5L)
  if (is.null(X)) {
    r2 <- pv <- numeric(0)
    log_msg("  PCA（by batch）略過：可用基因/樣本不足或仍含非有限值")
  } else {
...
COMBO_PREFIX <- if (!is.null(COMBO_MODE)) paste0("phospho_", COMBO_MODE) else NULL
...
## 本輪實際要跑與可用的資料集清單（存在資料夾且含 data_phosphoprotein_quantification.txt）
...
if (!length(dataset_dirs_run)) stop("dataset_dirs_run 為空，請確認資料夾與 data_phosphoprotein_quantification.txt 是否存在")
...
# --- [NEW] phospho 版矩陣載入（以 gene symbol 匯總） -----------------------------
load_phospho_matrix_from_dataset_dir <- function(dir,
  ...
  if (!file.exists(fp)) stop("[phospho] 檔案不存在：", fp)
  
  suppressMessages({
    df <- readr::read_tsv(fp, show_col_types = FALSE, progress = FALSE)
  })
  
  # 兼容欄位：ENTITY_STABLE_ID, NAME, DESCRIPTION, GENE_SYMBOL, PHOSPHOSITES, <samples...>
  meta_cols <- intersect(colnames(df),
                         c("ENTITY_STABLE_ID","NAME","DESCRIPTION","GENE_SYMBOL","PHOSPHOSITES"))
  sample_cols <- setdiff(colnames(df), meta_cols)
  if (!length(sample_cols)) stop("[phospho] 找不到樣本欄位")
  
  # --- site × sample 矩陣（以 GENE_SYMBOL 作為群組鍵），值已為 log2 ratio（CPTAC 標準）
  M_site <- as.matrix(df[, sample_cols, drop = FALSE])
  storage.mode(M_site) <- "numeric"
  rownames(M_site) <- df$GENE_SYMBOL
  
  # --- 匯總成 gene × sample（與 protein 版介面一致）
  split_idx <- split(seq_len(nrow(M_site)), rownames(M_site))
  agg_fun <- if (site_collapse == "median") function(x) stats::median(x, na.rm = TRUE) else
  ...
  if (isTRUE(protein_adjust) && file.exists(protein_fp)) {
    ...
    if (length(g_common) >= 1 && length(s_common) >= 3) {
      ...
      Y_adj <- Y
      for (g in g_common) {
        y <- as.numeric(Y[g, ]); x <- as.numeric(X[g, ])
        ok <- is.finite(y) & is.finite(x)
        if (sum(ok) >= 3) {
          fit <- stats::lm(y[ok] ~ x[ok])
          res <- rep(NA_real_, length(y)); res[ok] <- stats::residuals(fit)
          Y_adj[g, ] <- res
        } else {
          Y_adj[g, ] <- y
        }
  ...
  # 樣本命名微清理（與 protein 版一致）
  colnames(M_gene) <- gsub("\\s+", "", colnames(M_gene))
  if (exists("log_msg")) log_msg("【phospho】矩陣維度：%d genes × %d samples", nrow(M_gene), ncol(M_gene))
  return(M_gene)
}
...
run_imputation_test <- function(ds_id,
  ...
  batch_all   <- { bi <- get_batch_factor_phospho(ds_dir, colnames(mat0)); if (is.null(bi)) NULL else droplevels(bi$fac[colnames(mat0)]) }
  ...
  if (length(present_sub) >= 3) {
    if (exists("build_csn_score_safe", mode = "function")) {
      csn_score <- build_csn_score_safe(prot0, subunits = present_sub, combine_7AB = TRUE, min_members = 5L, pca_min_samples = 10L)
    } else if (exists("build_csn_score", mode = "function")) {
      csn_score <- build_csn_score(prot0, subunits = present_sub)
    } else {
      csn_score <- colMeans(prot0[present_sub, , drop = FALSE], na.rm = TRUE)
    }
  }
  
  # 準備 predictors：subunits / CSN_SCORE / RESIDUAL_<SU>
  predictors <- list()
  for (su in present_sub) predictors[[su]] <- prot0[su, ]
...
audit_one_dataset_sa_batch <- function(ds_dir, pipe_policy = "NA", min_per_level = 2) {
  ...
  # 4) batch 檢查（用你現有的 get_batch_factor_phospho）
...
## =========================================================
meta_fdr_stouffer(
  dataset_dirs = dataset_dirs_run,                 # 本輪實際跑過且可用的集合
  strata       = strata,                           # c("ALL","TP53_mutant","TP53_wild_type")
  stat_tags    = c("GSEA_limma_t_cont",
                   "GSEA_spearman",
                   "GSEA_limma_interaction"),           # 只彙整你實際產出的 site-level 指標
  groups       = names(genesets_by_group_ptm),     # ★ 改成 PTMsigDB（site-level）集合
...
csn_pairwise_correlation_one_ds <- function(
  ...
  # 讀矩陣
  mat0 <- load_phospho_matrix_from_dataset_dir(ds_dir)  # 你現成的讀檔器
  present <- intersect(subunits, rownames(mat0))
  if (length(present) < 2) {
  ...
  # 批次（若有），並對齊樣本
  bi <- get_batch_factor_phospho(ds_dir, sam_all)   # 既有
  batch <- NULL
...
## == Y 軸長 label 與圖面參數（可用 options 覆寫）==
options(
  csn_ylabel_wrap         = 40,   # 每行約 N 字元自動換行（dataset & meta 皆用）
  csn_left_margin_pts     = 160,  # 左邊界（pt），給長標籤空間
  csn_ylabel_extra_width  = 7.5,  # 額外加到自動寬度（in）
  
  ## Dataset 熱圖固定尺寸（若為 NA 則使用自動計算）
  csn_ds_heatmap_width_in  = NA_real_,
  csn_ds_heatmap_height_in = NA_real_,
  
  ## Meta-FDR 熱圖固定尺寸（若為 NA 則使用自動計算）
  csn_meta_heatmap_width_in  = NA_real_,
  csn_meta_heatmap_height_in = NA_real_,
  
  ## 輸出解析度
  csn_heatmap_dpi      = 600,   # dataset 熱圖
  csn_meta_heatmap_dpi = 600    # meta-FDR 熱圖
)

# 回傳 list(width=, height=)
...
.make_heatmap_plot <- function(csv_file) {
  ...
  if (!identical(toupper(.coll_tok), "H") && "NES_CSN_SCORE" %in% present_preds) {
    ...
    # [NEW] optional pathway prefix filter for single-dataset heatmap
  ...
  if ("NES_CSN_SCORE" %in% present_preds) {
    ...
    path_order <- df_long %>% dplyr::group_by(.data$pathway) %>%
...
run_gsea_heatmaps_for_all_datasets <- function(dataset_dirs_map = NULL,
  ...
  on.exit(options(csn_ds_heatmap_width_in  = old_w,
                  csn_ds_heatmap_height_in = old_h,
                  csn_heatmap_dpi          = old_dp), add = TRUE)
  ...
  for (ds in names(dataset_dirs_map)) {
    ...
    for (csv in csvs) {
      ...
      if (inherits(h, "try-error")) {
        message("[heatmap] 失敗：", csv, " | ", as.character(h)); next
      }
          ...
          if (!inherits(y_order, "try-error")) {
            ...
            if (!inherits(h2, "try-error")) {
              .save_near_only(h2$plot, h2$width, h2$height, csv, meta,
                              suffix = "_ordered",
                              dpi = getOption("csn_heatmap_dpi", 600))
              message("[heatmap][interaction-ordered] 完成：", csv)
            }
          }
        }
      }
      ...
      if (tolower(meta$stratum) %in% c("tp53_mutant","tp53_wild_type")) {
        ...
        if (is_cont || is_spearman) {
          ...
          if (length(sidx)) {
            parts2 <- parts; parts2[sidx[length(sidx)]] <- "ALL"
...
options(csn.dot_size_dataset = 2.0)  # 單一 dataset 熱圖圓點大小

## ---- 執行（例）----
run_gsea_heatmaps_for_all_datasets(
  dataset_dirs_map = if (exists("dataset_dirs_run")) dataset_dirs_run else NULL,
  collect_root = if (is.null(COMBO_PREFIX)) "single_dataset_GSEA_heatmap" else
...
.meta_make_heatmap_plot <- function(csv_file, y_order = NULL,
  ...
  if (is.null(y_order)) {
    if (!identical(toupper(.coll_tok), "H") && "Z_CSN_SCORE" %in% present) {
      ...
      keep_up <- csn_tbl %>% dplyr::arrange(dplyr::desc(.data$Z)) %>% dplyr::slice_head(n = .top_n) %>% dplyr::pull(.data$pathway)
      keep_dn <- csn_tbl %>% dplyr::arrange(.data$Z)                %>% dplyr::slice_head(n = .bot_n) %>% dplyr::pull(.data$pathway)
      keep <- unique(c(keep_up, keep_dn))
      if (length(keep)) df_long <- dplyr::filter(df_long, .data$pathway %in% keep)
    }
  }
  # [NEW] optional pathway prefix filter for meta heatmap
  .keep_prefix <- getOption("HEATMAP_PATHWAY_PREFIX", NULL)
  if (!is.null(.keep_prefix) && length(.keep_prefix) > 0) {
    .pat <- paste0("^(", paste(.keep_prefix, collapse = "|"), ")")
    df_long <- df_long %>% dplyr::filter(grepl(.pat, .data$pathway))
...
run_meta_fdr_heatmaps <- function(root = if (is.null(COMBO_PREFIX))
  ...
  on.exit(options(csn_meta_heatmap_width_in  = old_w,
                  csn_meta_heatmap_height_in = old_h,
                  csn_meta_heatmap_dpi       = old_dp), add = TRUE)
  ...
  if (!length(csvs)) { message("[meta-heatmap] 找不到目標，略過。"); return(invisible(TRUE)) }
  ...
  for (csv in csvs) {
    ...
    h <- try({
      if (is_interaction) .meta_make_heatmap_plot(csv, y_order = NULL, palette = pal_cell)
      else                .meta_make_heatmap_plot(csv)
    }, silent = TRUE)
    if (inherits(h, "try-error")) { message("[meta-heatmap] 建圖失敗：", csv); next }
    
    .meta_save_near_v3_shortname(h$plot, h$width, h$height, csv, meta,
                                 suffix = "",
                                 dpi = getOption("csn_meta_heatmap_dpi", 600))
    
    ## ordered cases（原規則不變）... 這裡呼叫的 h2/h3 也會回傳 width/height
...
options(csn.dot_size_meta    = 2.0)  # PAN/meta 熱圖圓點大小

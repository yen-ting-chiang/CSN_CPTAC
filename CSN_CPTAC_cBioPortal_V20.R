...
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group){
  ...
  ## === NEW === 在 ALL 分析：準備 TP53 狀態（0/1；mutant=1），供 SVA 保護與下游共變項使用
  is_ALL <- identical(basename(out_root), "ALL")
  tp53_num_all <- NULL
  if (is_ALL) {
    tp53_status_all <- get_tp53_status(ds_dir, colnames(mat0))  # "TP53_wild_type"/"TP53_mutant"
  ...
  ## 4) 逐 subunit 產生排名並執行 fgsea
  for (su in csn_subunits) {
    if (!(su %in% present_sub)) next
    log_msg("      >> 次單元：%s", su)
    sub_raw <- mat0[su, ]
    
    stats_t_RAW <- NULL; stats_t_ADJ <- NULL
    stats_rho_RAW <- NULL; stats_rho_ADJ <- NULL
    nH <- NA_integer_; nL <- NA_integer_
    ...
    if (length(v) < (2 * min_per_group)) {
      ...
      keep_s <- names(sub_raw)[grp != "Mid" & !is.na(sub_raw)]
      nH <- sum(grp[keep_s] == "High"); nL <- sum(grp[keep_s] == "Low")
      if (nH == 0 || nL == 0) {
        ...
        idx_low  <- head(ord_asc,  k); idx_high <- head(ord_desc, k)
        grp <- setNames(rep("Mid", length(sub_raw)), names(sub_raw))
        grp[idx_low]  <- "Low"; grp[idx_high] <- "High"
        keep_s <- union(idx_low, idx_high)
        nH <- sum(grp[keep_s] == "High"); nL <- sum(grp[keep_s] == "Low")
        log_msg("        ties → 改用排名：High=%d, Low=%d", nH, nL)
      }
      if (length(keep_s) >= (2 * min_per_group) && nH >= min_per_group && nL >= min_per_group) {
        grp2 <- factor(ifelse(grp[keep_s] == "High","High","Low"), levels = c("Low","High"))
        mat_limma <- mat[setdiff(rownames(mat), su), keep_s, drop = FALSE]
        
        ## RAW
        stats_t_RAW <- tryCatch(
          limma_t_with_covars(mat_limma, grp2),
          error = function(e){ log_msg("        limma RAW 失敗：%s", e$messsage); NULL }
        )
        
        ## ====== LIMMA 共變數（base + tech/SV + TP53；並把 TP53 放到 SVA 的 interest） ======
        ...
        ## === NEW === ALL 時把 TP53 加入下游共變項的候選
        if (is_ALL) {
          base_covars_raw$TP53_mutant <- as.numeric(tp53_num_all[keep_s])
        }
        ## === NEW END ===
        
        audit_covars_coverage("limma-base:before-select", ds_id, basename(out_root), su,
                              keep_s, batch_subset, base_covars_raw, sv = NULL, tech = NULL)
        
        covars <- select_covars_safely(base_covars_raw, keep_s, label = "limma-base",
        ...
        if (include_tech) {
          nuis_mm <- cbind(if (!is.null(batch_subset)) model.matrix(~ 0 + batch_subset) else NULL,
                           covars)
          
          # SVA interest：原本的 High/Low；=== NEW === 加上 TP53（ALL only）
          ...
          audit_design_alignment(
            tag      = sprintf("%s|%s|%s|limma:preSVA", ds_id, basename(out_root), su),
          ...
          svs <- estimate_svs(
            ...
            mod_interest = mod_interest_l,
            mod_nuisance = nuis_mm,
            label        = sprintf("%s|%s|%s|limma", ds_id, basename(out_root), su)
          )
          
          audit_sva_sanity(M = mat_limma, sv = svs,
          ...
          tech <- if (is.null(svs)) {
            ...
            gate_tech_pcs(tmp, as.numeric(grp2 == "High"))
          } else svs
          ...
          if (!is.null(tech)) {
            ...
            log_msg("  [tech] 無可用 SV/PC（已被 gate 或維度為 0）")
          }
          audit_covars_coverage("limma-tech", ds_id, basename(out_root), su,
                                keep_s, batch_subset, covars,
                                sv = svs, tech = tech)
        }
        
        # 最後保守篩選（避免 tech 與生物訊號高度相依）
        covars <- select_covars_safely(covars, keep_s, label = "limma-final",
                                       y = as.numeric(grp2 == "High"),
                                       min_cov_named = c(purity = 0.60, sex = 0.80, age = 0.80),
                                       max_abs_cor = 0.30)
        ## === NEW === 強制把 TP53 放入下游 limma 共變項（ALL only；避免被前述選擇程序剔除）
        if (is_ALL) {
          tp53_c <- matrix(as.numeric(tp53_num_all[keep_s]), ncol = 1,
                           dimnames = list(keep_s, "TP53_mutant"))
          if (is.null(covars)) covars <- tp53_c
          else if (!"TP53_mutant" %in% colnames(covars)) covars <- cbind(covars, tp53_c)
        }
        ## === NEW END ===
        
        audit_covars_coverage("limma-final:design-ready", ds_id, basename(out_root), su,
                              keep_s, batch_subset, covars,
                              sv = svs, tech = tech)
        
        log_msg("  [limma covars cols] %s",
                if (is.null(covars)) "NULL" else paste(colnames(covars), collapse = ","))
        log_msg("  [limma batch levels] %s",
                if (is.null(batch_subset)) "NULL" else paste(levels(batch_subset), collapse = ","))
        
        stats_t_ADJ <- tryCatch(
        ...
        limma_design_cols <- if (!is.null(stats_t_ADJ)) attr(stats_t_ADJ, "design_cols") else NA_character_
        
      } else {
        log_msg("        limma 條件不足，略過（High=%d, Low=%d）", nH, nL)
      }
    }
    
    ## ---- SPEARMAN （removeBatchEffect + Spearman）----
    ...
    ## === NEW === ALL 時把 TP53 放入 Spearman 的 base covariates
    if (is_ALL) {
      covars_all_base$TP53_mutant <- as.numeric(tp53_num_all[colnames(mat0)])
    }
    ## === NEW END ===
    
    audit_covars_coverage("spearman-base:before-select", ds_id, basename(out_root), su,
                          colnames(mat0), batch_all, covars_all_base,
                          sv = NULL, tech = NULL)
    
    covars_all <- select_covars_safely(covars_all_base, colnames(mat0), label = "spearman-base",
                                       y = as.numeric(sub_raw),
                                       min_cov_named = c(purity = 0.60, sex = 0.80, age = 0.80),
                                       max_abs_cor = 0.30)
    audit_covars_coverage("spearman-base:after-select", ds_id, basename(out_root), su,
    ...
    if (include_tech_all) {
      nuis_mm_all <- cbind(if (!is.null(batch_all)) model.matrix(~ 0 + batch_all) else NULL,
                           covars_all)
      
      ## interest：subunit raw；=== NEW === ALL 時加上 TP53
      sub_raw <- sub_raw[colnames(mat)]  # 對齊
      ...
      log_msg("[sanity] spearman rows: interest=%d, nuisance=%s; mat_cols=%d",
              nrow(mod_interest_s),
              if (!is.null(nuis_mm_all)) nrow(nuis_mm_all) else NA_integer_,
              ncol(mat))
      
      audit_design_alignment(
        tag      = sprintf("%s|%s|%s|spearman:preSVA", ds_id, basename(out_root), su),
      ...
      svs_all <- estimate_svs(
        ...
        mod_interest = mod_interest_s,
        mod_nuisance = nuis_mm_all,
        label        = sprintf("%s|%s|%s|spearman", ds_id, basename(out_root), su)
      )
      
      audit_sva_sanity(M = mat, sv = svs_all,
      ...
      if (!is.null(tech_all)) {
        ...
        audit_covars_coverage("spearman-tech", ds_id, basename(out_root), su,
                              colnames(mat0), batch_all, covars_all,
                              sv = svs_all, tech = tech_all)
      } else {
        log_msg("  [tech_all] 無可用 SV/PC（已被 gate 或維度為 0）")
        audit_covars_coverage("spearman-tech", ds_id, basename(out_root), su,
                              colnames(mat0), batch_all, covars_all,
    ...
    # 最後保守篩選（避免技術 covars 與生物訊號高度相依）
    covars_all <- select_covars_safely(covars_all, colnames(mat0), label = "spearman-final",
                                       y = as.numeric(sub_raw),
                                       min_cov_named = c(purity = 0.60, sex = 0.80, age = 0.80),
                                       max_abs_cor = 0.30)
    ## === NEW === 強制把 TP53 放入 Spearman 共變項（ALL only）
    if (is_ALL) {
      ...
      else if (!"TP53_mutant" %in% colnames(covars_all)) covars_all <- cbind(covars_all, tp53_all_c)
    }
    ## === NEW END ===
    
    audit_covars_coverage("spearman-final:design-ready", ds_id, basename(out_root), su,
                          colnames(mat0), batch_all, covars_all,
                          sv = svs_all, tech = tech_all)
    
    log_msg("  [spearman covars_all cols - FINAL] %s",
            if (is.null(covars_all)) "NULL" else paste(colnames(covars_all), collapse = ","))
    log_msg("  [spearman batch levels] %s",
            if (is.null(batch_all)) "NULL" else paste(levels(batch_all), collapse = ","))
    
    mat0_no_su <- mat0[setdiff(rownames(mat0), su), , drop = FALSE]
    
    stats_rho_ADJ <- tryCatch(
      spearman_rank_with_covars(
        mat_raw = mat0_no_su, sub_raw, batch = batch_all, covars = covars_all,
        min_pairs = min_pairs_spearman, impute_for_rbe = TRUE),
      error = function(e){ log_msg("        Spearman BatchAdj 無法進行：%s", e$message); NULL }
    )
    spearman_covar_cols <- if (!is.null(stats_rho_ADJ)) attr(stats_rho_ADJ, "covar_cols") else NA_character_
    
    ## ---- 寫檔（RAW/BatchAdj）----
    out_roots <- list(RAW = file.path(out_root, "RAW"),
                      BatchAdj = file.path(out_root, "BatchAdj"))
    for (ver in names(out_roots)) dir.create(file.path(out_roots[[ver]], su), recursive = TRUE, showWarnings = FALSE)
    
    write_one <- function(stats_list, ver_tag){
      if (is.null(stats_list$limma) && is.null(stats_list$spearman)) return(invisible(NULL))
      out_dir <- file.path(out_roots[[ver_tag]], su)
      for (grp_name in names(genesets_by_group)) {
        subdir <- file.path(out_dir, safe_fs_name(grp_name)); dir.create(subdir, recursive = TRUE, showWarnings = FALSE)
        if (!is.null(stats_list$limma)) {
          run_fgsea_save(stats = stats_list$limma, genesets = genesets_by_group[[grp_name]],
                         out_prefix = file.path(subdir, "GSEA_limma_t"),
                         top_plot_n = 0,
                         plot_title = glue::glue("{ds_id} | {su} | High vs Low | {grp_name} | {ver_tag}"))
        }
        if (!is.null(stats_list$spearman)) {
          run_fgsea_save(stats = stats_list$spearman, genesets = genesets_by_group[[grp_name]],
                         out_prefix = file.path(subdir, "GSEA_spearman"),
                         top_plot_n = 0,
                         plot_title = glue::glue("{ds_id} | {su} | Spearman | {grp_name} | {ver_tag}"))
        }
      }
    }
    ...
    counts_row <- data.frame(
      ...
      spearman_nonNA = sum(!is.na(sub_raw)),
      stringsAsFactors = FALSE
    )
    counts_fp <- file.path(out_root, "sample_counts.csv")
    data.table::fwrite(counts_row, counts_fp, append = file.exists(counts_fp))
    
    note_fp <- file.path(out_roots$BatchAdj, su, "batch_correction_notes.txt")
    info <- c(
      sprintf("dataset=%s; subunit=%s", ds_id, su),
      sprintf("batch_column=%s; n_levels=%s", if (!is.null(bi_all)) bi_all$name else NA, if (!is.null(batch_all)) nlevels(batch_all) else NA),
      ...
      "spearman: removeBatchEffect(batch, covariates=SV or PC1-2 + optional purity/sex/age + TP53 in ALL; design=~subunit_raw (+ TP53 protected in SVA))",
      ...
      "Notes:",
      "  - 在 ALL：TP53 放進 SVA 的 mod（保護）與下游共變項（校正）。在分層（MUT/WT）內不加入（常數）。",
      "  - TMT/CPTAC 情境下，不將 per-sample median/missrate 當共變數，只用於 QC；",
      "  - 優先用 SVA 的 SV；抓不到才使用 gated PC1/PC2；",
      "  - purity/sex/age 僅在覆蓋率足夠且不貼近生物訊號時納入（自動判斷）。"
    )

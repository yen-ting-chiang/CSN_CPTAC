...
run_and_write_gsea_spearman <- function(stats_named_vec, pathways_H,
                                        out_dir_root, subunit, pass_label) {
  stopifnot(is.numeric(stats_named_vec), !is.null(names(stats_named_vec)))
  out_dir <- file.path(out_dir_root, subunit, "H")
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  
  minSize <- opt("minSize", 15L)
  maxSize <- opt("maxSize", 500L)
  
  orig_n <- length(pathways_H)
  pw_use <- ._intersect_and_filter_pathways(pathways_H, names(stats_named_vec), minSize = minSize, maxSize = maxSize)
  message(sprintf("[gsea:%s] %s |H| 原=%d → 用=%d", pass_label, subunit, orig_n, length(pw_use)))
...
.gsea_from_ranks <- function(pathways, stats, minSize, maxSize, gsea_eps = 0, label = NULL) {
  ...
  message(sprintf("[gsea:%s] |H| 原=%d → 用=%d", ifelse(is.null(label), "NA", label), orig_n, length(pw_use)))
...
gsea_from_diffcorr <- function(pathways, z_stats, label = "diff-corr", out_prefix = NULL) {
  ...
  if (!is.null(out_prefix)) {
    message(sprintf("[gsea:%s] %s |H| 原=%d → 用=%d", label, out_prefix, orig_n, length(pw_use)))
  } else {
    message(sprintf("[gsea:%s] |H| 原=%d → 用=%d", label, orig_n, length(pw_use)))
  }
...
run_fgsea_save <- function(stats, genesets, out_prefix, top_plot_n = 0, plot_title = "") {
  ...
  message(sprintf("[gsea] %s |H| 原=%d → 用=%d", out_prefix, orig_n, length(pw_use)))
  if (!length(pw_use)) {
...
read_gsea_table <- function(out_root, subunit, group_name, stat_tag) {
  ...
  candidates <- c(
    file.path(out_root, grp, su, "H", f),  # 現在：.../<group>/<subunit>/H/STAT.csv
    file.path(out_root, grp, su, f),       # 沒有 H 子夾時
    file.path(out_root, su, grp, "H", f),  # 舊：.../<subunit>/<group>/H/STAT.csv
    file.path(out_root, su, grp, f)        # 舊：沒有 H 子夾
  )
      ...
      for (stat_tag in stat_tags) {
        for (grp in groups) {
          ...
          subunits <- unique(unlist(lapply(dataset_dirs, function(dsdir) {
            base <- file.path(dsdir, "csn_gsea_results_TP53", stratum, pass_label)
          ...
          for (su in subunits) {
            ...
            for (dsdir in dataset_dirs) {
              f <- file.path(dsdir, "csn_gsea_results_TP53", stratum, pass_label,
                             su, grp, sprintf("%s.csv", stat_tag))
...
estimate_svs <- function(M, mod_interest, mod_nuisance = NULL, max_k = 5, label = "") {
  ...
  tryCatch({
    ...
    ## NEW: make a finite copy for SVA engines
    M_finite <- M
    ...
    mod0 <- cbind("(Intercept)" = rep(1, nrow(mod_nuisance)), mod_nuisance)
    qr0  <- qr(mod0)
    ...
    if (k == 0) {
      ## 用「與 SVA 相同的設計」做殘差 PCA，量化是否有明顯潛在結構
      ve1 <- 0
      ...
      if (!is.null(R)) {
        ...
        if (!is.null(pc)) {
          ...
          ve1 <- if (length(ve)) ve[1] else 0
          log_msg("[SVA%s] residual PCA var%% (top5) = %s", tag,
                  paste(round(ve[seq_len(min(5, length(ve)))], 1), collapse = ", "))
        }
      }
      if (is.finite(ve1) && ve1 >= 5 && res_df >= 2) {
        k <- as.integer(min(2L, res_df - 1L, max_k))
        log_msg("[SVA%s] num.sv→k=0，但殘差PC1=%.1f%%，強制使用 k=%d 重新估計", tag, ve1, k)
        ## 不 return；直接往下走既有的引擎選擇（svaseq/sva）與估計流程
      } else {
        log_msg("[SVA%s] k=0 且殘差PC1不足以支持（%.1f%%）→ 放棄 SVA", tag, ve1)
        return(NULL)
      }
    }
    ...
    if (engine == "svaseq") {
      sv <- tryCatch(
        sva::svaseq(dat = M_finite, mod = as.matrix(mod), mod0 = as.matrix(mod0), n.sv = k)$sv,
        error = function(e){
...
run_predictor_analyses <- function(
  ...
  audit_covars_coverage(
    tag       = sprintf("%s|RAW", predictor_name),
    ds_id     = ds_id,
  ...
  if (!is.null(genesets_by_group[["H"]])) {
    # RAW
    try({
      log_msg("[spearman:RAW] start; subunit=%s", predictor_name)
      expr_raw <- as.matrix(mat0[, colnames(M_raw), drop = FALSE])
      pred_raw <- pred[rownames(DF_raw)]; names(pred_raw) <- rownames(DF_raw)   # 未中心化
      stats_raw_sp <- spearman_prerank(expr_raw,
                                       predictor   = pred_raw,
                                       min_non_na  = max(16L, get0("min_pairs_spearman", ifnotfound = 16L)))
      ## NEW: RAW spearman—若統計量不可用就清楚記 log，否則才寫檔
      if (is.null(stats_raw_sp) || !length(stats_raw_sp) || all(!is.finite(stats_raw_sp))) {
        log_msg("[spearman:RAW] skip: empty/NA stats after QC (n=%s, finite=%s)",
                length(stats_raw_sp), sum(is.finite(stats_raw_sp)))
      } else {
        run_and_write_gsea_spearman(
          stats_raw_sp, genesets_by_group$H,
          out_dir_root = file.path(out_root, "RAW"),
    ...
    # BatchAdj（以 M_ba 表達；predictor 同樣未中心化）
    try({
      log_msg("[spearman:BatchAdj|partial] start; subunit=%s", predictor_name)
      ...
      run_and_write_gsea_spearman(stats_ba_sp, genesets_by_group$H,
                                  out_dir_root = file.path(out_root, "BatchAdj"),
                                  subunit      = predictor_name,
                                  pass_label   = "BatchAdj")
    }, silent = TRUE)
  }
  ...
  if (isTRUE(is_ALL) && !is.null(tp53_num_all) && !is.null(genesets_by_group[["H"]])) {
    ...
    try({
      ...
      if (nlevels(droplevels(tp53_raw_fac)) < 2) {
        ...
        if (!is.null(ne) && any(grepl("^(tp53|pred:tp53)", ne))) {
          ...
          if (coef_int %in% colnames(coef(fit_int_raw))) {
            ...
            pw_H <- genesets_by_group$H
            out_dir_int_raw <- file.path(out_root, "RAW", predictor_name, safe_fs_name("H"))
            dir.create(out_dir_int_raw, recursive = TRUE, showWarnings = FALSE)
            # 統一的 GSEA 包裝（交集+size；Multilevel → fallback；含 log）
            ...
            # camera
            idx <- limma::ids2indices(pw_H, names(ranks), remove.empty = TRUE)
            res_cam <- limma::cameraPR(ranks, idx); res_cam$pathway <- rownames(res_cam)
            data.table::fwrite(as.data.frame(res_cam), file.path(out_dir_int_raw, "GSEA_limma_interaction_camera.csv"))
            # mroast
            res_mr <- limma::mroast(y = M_raw, index = idx, design = des_int_raw,
                                    contrast = which(colnames(des_int_raw) == coef_int),
                                    nrot = 9999, set.statistic = "mean")
            res_mr <- as.data.frame(res_mr); res_mr$pathway <- rownames(res_mr)
      ...
      if (length(sWT) >= min_pairs && length(sMT) >= min_pairs) {
        ...
        if (n_wt >= 4 && n_mt >= 4) {
          ...
          idx2 <- limma::ids2indices(pw_H, names(z_diff), remove.empty = TRUE)
          res_fg2 <- gsea_from_diffcorr(pw_H, z_diff, label = "diff-corr")
          data.table::fwrite(as.data.frame(res_fg2), file.path(out_dir_int_raw, "GSEA_spearman_diffcorr.csv"))
          res_cam2 <- limma::cameraPR(z_diff, idx2); res_cam2$pathway <- rownames(res_cam2)
          data.table::fwrite(as.data.frame(res_cam2), file.path(out_dir_int_raw, "GSEA_spearman_diffcorr_camera.csv"))
        }
      }
    }, silent = TRUE)
    ...
    try({
      ...
      if (coef_int %in% colnames(coef(fit_int_ba))) {
        ...
        pw_H <- genesets_by_group$H
        out_dir_int_ba <- file.path(out_root, "BatchAdj", predictor_name, safe_fs_name("H"))
        ...
        idx <- limma::ids2indices(pw_H, names(ranks), remove.empty = TRUE)
        res_cam <- limma::cameraPR(ranks, idx); res_cam$pathway <- rownames(res_cam)
        data.table::fwrite(as.data.frame(res_cam), file.path(out_dir_int_ba, "GSEA_limma_interaction_camera.csv"))
        res_mr <- limma::mroast(y = M_ba, index = idx, design = des_int_ba,
      ...
      if (length(sWT) >= min_pairs && length(sMT) >= min_pairs) {
        ...
        if (n_wt >= 4 && n_mt >= 4) {
          ...
          res_cam2 <- limma::cameraPR(z_diff, idx2); res_cam2$pathway <- rownames(res_cam2)
          data.table::fwrite(as.data.frame(res_cam2), file.path(out_dir_int_ba, "GSEA_spearman_diffcorr_camera.csv"))
        }
      }
    }, silent = TRUE)
  }
  ...
  for (grp_name in names(genesets_by_group)) {
    ...
    ## A1
    out_dir_A1 <- file.path(out_root, "RAW", predictor_name, safe_fs_name(grp_name))
    ...
    # camera / mroast（敏感度）
    try(run_camera_from_ranks_save(t1, pw, out_dir_A1, outfile = "GSEA_camera.csv"), silent = TRUE)
    try(run_mroast_save(M_raw, des_raw, coef_name(des_raw), pw, out_dir_A1, outfile = "GSEA_mroast.csv"), silent = TRUE)
    
    ## A2
    out_dir_A2 <- file.path(out_root, "BatchAdj", predictor_name, safe_fs_name(grp_name))
    ...
    try(run_camera_from_ranks_save(t2, pw, out_dir_A2, outfile = "GSEA_camera.csv"), silent = TRUE)
    try(run_mroast_save(M_ba, des_ba, coef_name(des_ba), pw, out_dir_A2, outfile = "GSEA_mroast.csv"), silent = TRUE)
  }
}
...
options(csn.audit_sva_sanity = TRUE)  # 想關掉就設 FALSE
...
tp53_delta_nes_aggregate <- function(datasets_root,
  ...
  for (ds in dataset_ids) {
    ...
    for (ver in versions) {
      ...
      subunits <- unique(c(list.dirs(file.path(base_dir, "TP53_wild_type", ver), full.names = FALSE, recursive = FALSE),
                           list.dirs(file.path(base_dir, "TP53_mutant",    ver), full.names = FALSE, recursive = FALSE)))
      ...
      for (su in subunits) {
        for (grp in groups) {
          ...
          for (st in stat_tags) {
            fp_wt <- file.path(base_dir, "TP53_wild_type", ver, su, grp_safe, paste0(st, ".csv"))
            fp_mt <- file.path(base_dir, "TP53_mutant",    ver, su, grp_safe, paste0(st, ".csv"))
            ...
            out_dir <- file.path(base_dir, "DeltaWT_MT", ver, su, grp_safe)
            dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
...
meta_fdr_stouffer(
  ...
  stat_tags = c("GSEA_limma_t_cont","GSEA_spearman"), # 與輸出檔名一致
  groups = names(genesets_by_group),
  out_root = "csn_gsea_pan_summary_TP53/meta_fdr"
)
...
detect_subunits_tp53 <- function(dataset_dirs, strata, versions, groups) {
  blacklist <- c("summary", "run_info", "tables")  # 可再加
  subs <- list()
  for (ds in names(dataset_dirs)) {
    for (st in strata) for (ver in versions) {
      base <- file.path(dataset_dirs[[ds]], "csn_gsea_results_TP53", st, ver)
...
for (ver in versions) {
  ...
  data.table::fwrite(pan_by_class, file.path(pan_out_root, "pan_summary_by_subunit_class.csv"))

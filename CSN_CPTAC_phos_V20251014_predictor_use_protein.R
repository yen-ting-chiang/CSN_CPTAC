  ...
  if (isTRUE(protein_adjust) && file.exists(protein_fp)) {
    ...
    prot_meta <- intersect(colnames(prot_df),
                           c("Hugo_Symbol","GENE_SYMBOL","GeneSymbol","Description",
                             "GENE","GENE_ID","GENE.STABLE.ID"))
    prot_sample <- setdiff(colnames(prot_df), prot_meta)
    # 盡量找到 gene symbol 欄位
    gcol <- c("Hugo_Symbol","GENE_SYMBOL","GeneSymbol","GENE")[c("Hugo_Symbol","GENE_SYMBOL","GeneSymbol","GENE") %in% colnames(prot_df)][1]
...
run_imputation_test <- function(ds_id,
  ...
  # 載入矩陣（延續你現有載入/對數轉換邏輯）
  mat0_full   <- load_phospho_matrix_from_dataset_dir(ds_dir)

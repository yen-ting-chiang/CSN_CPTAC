...
## ============================================================
## [NEW | PART 1] GSEA Result Summary CSV Generator
## ============================================================
## This function consolidates GSEA results from all datasets into
## a single CSV file per stratum/predictor combination.
## Rows = datasets, Columns = signature NES and padj values
## ============================================================

generate_gsea_summary_csv <- function(
  base_dir = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "PTMsigDB", "BatchAdj"),
  output_base = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "GSEA_result_summary"),
  dataset_ids = c(
  ...
  strata = c("ALL", "TP53_mutant", "TP53_wild_type"),
  predictors = NULL,
  gsea_file = "GSEA_limma_t_cont.csv"
) {
  log_msg("[GSEA-summary] Starting GSEA result summary CSV generation...")
  log_msg("[GSEA-summary] Base directory: %s", base_dir)
  log_msg("[GSEA-summary] Output base: %s", output_base)

  ## Auto-detect predictors if not specified
  if (is.null(predictors)) {
    ## Scan first available dataset/stratum to get predictor list
    for (ds in dataset_ids) {
      for (st in strata) {
        scan_path <- file.path(base_dir, ds, st)
        if (dir.exists(scan_path)) {
          predictors <- list.dirs(scan_path, full.names = FALSE, recursive = FALSE)
          predictors <- predictors[predictors != "summary"]
          if (length(predictors) > 0) {
            log_msg("[GSEA-summary] Auto-detected predictors: %s", paste(predictors, collapse = ", "))
            break
          }
        }
      }
      if (!is.null(predictors) && length(predictors) > 0) break
    }
  }

  if (is.null(predictors) || length(predictors) == 0) {
    log_msg("[GSEA-summary] No predictors found, aborting.")
    return(invisible(NULL))
  }

  ## Process each stratum × predictor combination
  for (stratum in strata) {
    for (predictor in predictors) {
      log_msg("[GSEA-summary] Processing stratum=%s, predictor=%s", stratum, predictor)

      all_results <- list()
      all_signatures <- character(0)

      ## Collect data from each dataset
      for (ds in dataset_ids) {
        csv_path <- file.path(base_dir, ds, stratum, predictor, gsea_file)

        if (!file.exists(csv_path)) {
          log_msg("[GSEA-summary]   %s: File not found, skipping.", ds)
          next
        }
        ...
        df <- tryCatch(
          ...
          error = function(e) {
            log_msg("[GSEA-summary]   %s: Error reading file: %s", ds, conditionMessage(e))
            NULL
          }
        )
        ...
        if (!("pathway" %in% names(df)) && ("term" %in% names(df))) {
          names(df)[names(df) == "term"] <- "pathway"
        }

        if (!all(c("pathway", "nes", "padj") %in% names(df))) {
          log_msg("[GSEA-summary]   %s: Missing required columns, skipping.", ds)
          next
        }

        all_signatures <- union(all_signatures, df$pathway)
        all_results[[ds]] <- df
        log_msg("[GSEA-summary]   %s: Loaded %d signatures.", ds, nrow(df))
      }

      if (length(all_results) == 0) {
        log_msg("[GSEA-summary]   No data available for this combination, skipping.")
        next
      }

      ## Build summary data frame
      summary_rows <- lapply(dataset_ids, function(ds) {
        if (!ds %in% names(all_results)) {
          ## Dataset not available, fill with NA
          row_data <- data.frame(Dataset = ds, stringsAsFactors = FALSE)
          for (sig in all_signatures) {
            row_data[[paste0(sig, "_NES")]] <- NA_real_
            row_data[[paste0(sig, "_padj")]] <- NA_real_
          }
          return(row_data)
        }
        ...
        for (sig in all_signatures) {
          idx <- which(df$pathway == sig)
          if (length(idx) > 0) {
            row_data[[paste0(sig, "_NES")]] <- df$nes[idx[1]]
            row_data[[paste0(sig, "_padj")]] <- df$padj[idx[1]]
          } else {
            row_data[[paste0(sig, "_NES")]] <- NA_real_
            row_data[[paste0(sig, "_padj")]] <- NA_real_
          }
      ...
      ## Ensure dataset order matches input order
      summary_df$Dataset <- factor(summary_df$Dataset, levels = dataset_ids)
      summary_df <- summary_df[order(summary_df$Dataset), ]
      summary_df$Dataset <- as.character(summary_df$Dataset)

      ## Create output directory and save
      out_dir <- file.path(output_base, stratum, predictor)
      dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
      out_file <- file.path(out_dir, "GSEA_summary.csv")

      data.table::fwrite(summary_df, out_file)
      log_msg(
        "[GSEA-summary]   Saved: %s (%d datasets × %d signatures)",
        out_file, nrow(summary_df), length(all_signatures)
      )
    }
  }

  log_msg("[GSEA-summary] GSEA result summary CSV generation completed.")
  invisible(TRUE)
}


## ============================================================
## [NEW | PART 2] GSEA Bubble Plot Generator
## ============================================================
## Creates publication-quality bubble plots for GSEA results.
## X-axis: signatures, Y-axis: datasets
## Bubble color: NES (blue=negative, white=0, red=positive)
## Bubble size: -log10(padj)
## ============================================================

generate_gsea_bubble_plot <- function(
  base_dir = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "PTMsigDB", "BatchAdj"),
  output_dir = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "bubble_plot"),
  dataset_ids = c(
    "brca_cptac_2020", "coad_cptac_2019", "gbm_cptac_2021",
    "luad_cptac_2020", "paad_cptac_2021", "ucec_cptac_2020"
  ),
  ...
  predictor = "CSN_SCORE",
  signatures = c(
    "KINASE-PSP_PKACA/PRKACA", "KINASE-PSP_Akt1/AKT1",
    "PATH-NP_EGFR1_PATHWAY", "KINASE-PSP_CDK2"
  ),
  gsea_file = "GSEA_limma_t_cont.csv",
  color_low = "#2166AC",
  color_mid = "white",
  ...
  log_msg("[bubble-plot] Starting GSEA bubble plot generation...")
  log_msg("[bubble-plot] Stratum: %s, Predictor: %s", stratum, predictor)
  log_msg("[bubble-plot] Signatures: %s", paste(signatures, collapse = ", "))
  ...
  for (ds in dataset_ids) {
    ...
    if (!file.exists(csv_path)) {
      log_msg("[bubble-plot]   %s: File not found, skipping.", ds)
      next
    }
    ...
    df <- tryCatch(
      data.table::fread(csv_path, na.strings = c("NA", "NaN", "")),
      error = function(e) {
        log_msg("[bubble-plot]   %s: Error reading file: %s", ds, conditionMessage(e))
        NULL
      }
    )
    ...
    if (!all(c("pathway", "nes", "padj") %in% names(df))) {
      log_msg("[bubble-plot]   %s: Missing required columns, skipping.", ds)
      next
    }

    ## Filter to requested signatures
    df <- df[df$pathway %in% signatures, c("pathway", "nes", "padj"), drop = FALSE]

    if (nrow(df) > 0) {
      df$dataset <- ds
      plot_data[[ds]] <- df
      log_msg("[bubble-plot]   %s: Loaded %d signatures.", ds, nrow(df))
    }
  }

  if (length(plot_data) == 0) {
    log_msg("[bubble-plot] No data available, aborting.")
    return(invisible(NULL))
  }

  ## Combine all data
  combined_df <- dplyr::bind_rows(plot_data)

  ## Calculate -log10(padj) for bubble size
  combined_df$neg_log10_padj <- -log10(combined_df$padj)
  combined_df$neg_log10_padj[!is.finite(combined_df$neg_log10_padj)] <- 0
  combined_df$neg_log10_padj[combined_df$neg_log10_padj < 0] <- 0

  ## Set factor levels for ordering
  combined_df$dataset <- factor(combined_df$dataset, levels = rev(dataset_ids))
  combined_df$pathway <- factor(combined_df$pathway, levels = signatures)

  ## Determine NES color limits
  if (is.null(nes_limits)) {
    max_abs <- max(abs(combined_df$nes), na.rm = TRUE)
    if (!is.finite(max_abs) || max_abs == 0) max_abs <- 1
    nes_limits <- c(-max_abs, max_abs)
  }

  ## Create default title if not provided
  if (is.null(plot_title)) {
    plot_title <- sprintf("GSEA Results: %s stratum, %s predictor", stratum, predictor)
  }

  ## Build the bubble plot
  p <- ggplot2::ggplot(combined_df, ggplot2::aes(x = pathway, y = dataset)) +
    ggplot2::geom_point(
      ggplot2::aes(size = neg_log10_padj, fill = nes),
      shape = 21,
      color = "black",
      stroke = 0.5
    ) +
    ...
    ggplot2::scale_size_continuous(
      ...
      limits = c(0, max(combined_df$neg_log10_padj, na.rm = TRUE) * 1.1)
    ) +
    ggplot2::labs(
      title = plot_title,
      x = x_label,
    ...
    ggplot2::theme(
      ...
      axis.text.x = ggplot2::element_text(angle = 45, hjust = 1, vjust = 1, size = base_size - 1),
    ...
    ggplot2::guides(
      fill = ggplot2::guide_colorbar(order = 1, barwidth = 1.2, barheight = 8),
  ...
  if (is.null(output_prefix)) {
    output_prefix <- sprintf("GSEA_bubble_%s_%s", stratum, safe_fs_name(predictor))
  }

  ## Save TIFF (publication quality)
  tiff_file <- file.path(output_dir, paste0(output_prefix, ".tiff"))
  ggplot2::ggsave(
    tiff_file,
    plot = p,
  ...
  log_msg("[bubble-plot] Saved TIFF: %s", tiff_file)
  ...
  log_msg("[bubble-plot] Saved PDF: %s", pdf_file)

  log_msg("[bubble-plot] GSEA bubble plot generation completed.")

  invisible(list(plot = p, data = combined_df, tiff = tiff_file, pdf = pdf_file))
}


## ============================================================
## [NEW | PART 3] Execute GSEA Summary and Bubble Plot
## ============================================================

## --- Generate GSEA Summary CSV for all stratum/predictor combinations ---
log_msg("=== Generating GSEA Summary CSV files ===")
generate_gsea_summary_csv(
  base_dir = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "PTMsigDB", "BatchAdj"),
  output_base = file.path(COMBO_PREFIX %||% "phosphoproteomic_site_level_GSEA", "GSEA_result_summary"),
  dataset_ids = c(
    "brca_cptac_2020", "coad_cptac_2019", "gbm_cptac_2021",
  ...
  strata = c("ALL", "TP53_mutant", "TP53_wild_type"),
  predictors = NULL ## Auto-detect all available predictors
)

## --- Generate Bubble Plot for ALL stratum with CSN_SCORE predictor ---
log_msg("=== Generating GSEA Bubble Plot ===")

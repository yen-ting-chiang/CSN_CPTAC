...
GSEA_PREFIX <- "proteomic_GSEA_add_predictors"
...
run_predictor_analyses <- function(
  ...
  ## Include ALL columns in DF_ba to ensure complete.cases filters NA in extra_covars too
  use_cols <- colnames(DF_ba)
...
run_one_stratum <- function(ds_id, ds_dir, mat0_full, sample_keep, out_root, genesets_by_group) {
  ...
  ## 4) Perform analysis on a per-prediction vector basis: original value of subunit, CSN_SCORE, RESIDUAL_<SU>
  {
    ...
    ## 4a-NEW) COPS7A_not_B: COPS7A as predictor + COPS7B as covariate
    if ("COPS7A" %in% present_sub && "COPS7B" %in% present_sub) {
      cops7b_expr <- mat0["COPS7B", ]
      if (sum(is.finite(cops7b_expr)) >= (2 * min_per_group)) {
        ex_df_7b <- data.frame(COPS7B = cops7b_expr, row.names = names(cops7b_expr))
        run_predictor_analyses(
          predictor_name = "COPS7A_not_B",
          predictor_vec = mat0["COPS7A", ],
          exclude_genes = "COPS7A",
          ds_id = ds_id, ds_dir = ds_dir,
          mat0 = mat0, mat = impute_and_filter(mat0, min_frac = min_frac_complete),
          out_root = out_root,
          genesets_by_group = genesets_by_group,
          batch_all = batch_all, purity_all = purity_all, sa_all = sa_all_limma,
          tp53_num_all = tp53_num_all, is_ALL = is_ALL,
          extra_covars_df = ex_df_7b
        )
      } else {
        log_msg("  [COPS7A_not_B] COPS7B non-NA samples not enough, skip")
      }
    }

    ## 4a-NEW) COPS7B_not_A: COPS7B as predictor + COPS7A as covariate
    if ("COPS7A" %in% present_sub && "COPS7B" %in% present_sub) {
      cops7a_expr <- mat0["COPS7A", ]
      if (sum(is.finite(cops7a_expr)) >= (2 * min_per_group)) {
        ex_df_7a <- data.frame(COPS7A = cops7a_expr, row.names = names(cops7a_expr))
        run_predictor_analyses(
          predictor_name = "COPS7B_not_A",
          predictor_vec = mat0["COPS7B", ],
          exclude_genes = "COPS7B",
          ds_id = ds_id, ds_dir = ds_dir,
          mat0 = mat0, mat = impute_and_filter(mat0, min_frac = min_frac_complete),
          out_root = out_root,
          genesets_by_group = genesets_by_group,
          batch_all = batch_all, purity_all = purity_all, sa_all = sa_all_limma,
          tp53_num_all = tp53_num_all, is_ALL = is_ALL,
          extra_covars_df = ex_df_7a
        )
      } else {
        log_msg("  [COPS7B_not_A] COPS7A non-NA samples not enough, skip")
      }
    }

    ## 4a-NEW) COPS7A_and_B: Sum of COPS7A + COPS7B abundance
    if ("COPS7A" %in% present_sub && "COPS7B" %in% present_sub) {
      cops7a_expr <- mat0["COPS7A", ]
      cops7b_expr <- mat0["COPS7B", ]
      cops7_sum <- cops7a_expr + cops7b_expr
      names(cops7_sum) <- colnames(mat0)
      if (sum(is.finite(cops7_sum)) >= (2 * min_per_group)) {
        run_predictor_analyses(
          predictor_name = "COPS7A_and_B",
          predictor_vec = cops7_sum,
          exclude_genes = c("COPS7A", "COPS7B"),
          ds_id = ds_id, ds_dir = ds_dir,
          mat0 = mat0, mat = impute_and_filter(mat0, min_frac = min_frac_complete),
          out_root = out_root,
          genesets_by_group = genesets_by_group,
        ...
        log_msg("  [COPS7A_and_B] COPS7A+COPS7B sum non-NA samples not enough, skip")
      }
    }

    ## 4a-NEW) COPS7_A_B_ratio: Log-ratio of COPS7A vs COPS7B (COPS7A - COPS7B for log2 data = log2(A/B))
    if ("COPS7A" %in% present_sub && "COPS7B" %in% present_sub) {
      cops7a_expr <- mat0["COPS7A", ]
      cops7b_expr <- mat0["COPS7B", ]
      # For log2 ratio data: log2(A) - log2(B) = log2(A/B)
      cops7_ratio <- cops7a_expr - cops7b_expr
      names(cops7_ratio) <- colnames(mat0)
      if (sum(is.finite(cops7_ratio)) >= (2 * min_per_group)) {
        run_predictor_analyses(
          predictor_name = "COPS7_A_B_ratio",
          predictor_vec = cops7_ratio,
          exclude_genes = c("COPS7A", "COPS7B"),
          ds_id = ds_id, ds_dir = ds_dir,
          mat0 = mat0, mat = impute_and_filter(mat0, min_frac = min_frac_complete),
          out_root = out_root,
        ...
        log_msg("  [COPS7_A_B_ratio] COPS7A-COPS7B difference non-NA samples not enough, skip")
      }
    }
  ...
  present_sub <- intersect(csn_subunits, rownames(mat0))
  sum_units <- c(present_sub, "COPS7A_not_B", "COPS7B_not_A", "COPS7A_and_B", "COPS7_A_B_ratio", "CSN_SCORE", paste0("RESIDUAL_", present_sub))
...
.pred_order_all <- c(
  ...
  "NES_COPS7A", "NES_COPS7B", "NES_COPS7A_not_B", "NES_COPS7B_not_A", "NES_COPS7A_and_B", "NES_COPS7_A_B_ratio", "NES_COPS8", "NES_COPS9",
  "NES_RESIDUAL_GPS1",

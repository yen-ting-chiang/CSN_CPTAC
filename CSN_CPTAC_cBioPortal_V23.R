...
meta_fdr_stouffer <- function(dataset_dirs, strata = c("ALL","TP53_mutant","TP53_wild_type"),
  ...
  predictors_by_ds <- lapply(dataset_dirs, function(ds_dir){
    base <- file.path(ds_dir, "csn_gsea_results_TP53")
    if (!dir.exists(base)) return(character(0))
    uniq <- c()
    for (st in strata) {
      dd <- file.path(base, st, "BatchAdj")
      if (dir.exists(dd)) {
        kids <- list.dirs(dd, full.names = FALSE, recursive = FALSE)
        uniq <- union(uniq, kids)
      }
    }
    uniq
  })
  predictors <- sort(unique(unlist(predictors_by_ds)))
  
  for (st in strata) for (stat_tag in stat_tags) for (grp in groups) for (pred in predictors) {
    rows <- list(); j <- 1L
    for (ds in names(dataset_dirs)) {
      base <- file.path(dataset_dirs[[ds]], "csn_gsea_results_TP53", st, "BatchAdj", pred, safe_fs_name(grp))
      fp <- file.path(base, paste0(stat_tag, ".csv"))
      dt <- safe_read_gsea(fp); if (is.null(dt)) next
      dt$dataset <- ds
      rows[[j]] <- dt; j <- j + 1L
    }
    if (!length(rows)) next
    ...
    comb <- df |>
      ...
      dplyr::summarise(
        k = dplyr::n(),
        z = { z_i <- .z_from_p_signed(pval, sign = sign(NES)); sum(z_i) / sqrt(dplyr::n()) },
        .groups = "drop"
      )
    comb$p_meta <- 2 * (1 - stats::pnorm(abs(comb$z)))
    comb$FDR_meta <- p.adjust(comb$p_meta, method = "BH")
    
    dir_tbl <- df |>
      dplyr::group_by(pathway) |>
      dplyr::summarise(pos_n = sum(NES > 0, na.rm=TRUE),
                       neg_n = sum(NES < 0, na.rm=TRUE),
                       .groups = "drop")
    out_df <- comb |> dplyr::left_join(dir_tbl, by = "pathway") |>
      dplyr::arrange(FDR_meta, dplyr::desc(abs(z)))
    
    odir <- file.path(out_root, st, safe_fs_name(grp))
    dir.create(odir, recursive = TRUE, showWarnings = FALSE)
    ofp_csv  <- file.path(odir, sprintf("%s__%s__meta_stouffer.csv", pred, stat_tag))
    ofp_xlsx <- file.path(odir, sprintf("%s__%s__meta_stouffer.xlsx", pred, stat_tag))
    data.table::fwrite(out_df, ofp_csv)
    wb <- openxlsx::createWorkbook(); openxlsx::addWorksheet(wb, "meta")
    openxlsx::writeData(wb, 1, out_df); openxlsx::saveWorkbook(wb, ofp_xlsx, overwrite = TRUE)
    log_msg("[meta] wrote %s (%s | %s | %s)", basename(ofp_csv), st, pred, stat_tag)
  }
}
...
limma_cont_with_covars <- function(mat, x, batch = NULL, covars = NULL) {
  stopifnot(!is.null(colnames(mat)))
  sam <- colnames(mat)
  ...
  des_x <- model.matrix(~ x)    # (Intercept), x
  colnames(des_x) <- c("(Intercept)","x")
  ...
  if (!is.null(covars)) {
    cv <- .align_to_samples(covars, sam, what = "covars")
    cv <- as.matrix(cv); if (!is.null(cv) && nrow(cv) == length(sam) && ncol(cv) > 0) Xc <- cv
  }
  if (!is.null(Xc)) Xc[!is.finite(Xc)] <- 0
  
  parts <- list(des_x)
  if (!is.null(Xb) && nrow(Xb) == length(sam) && ncol(Xb) > 0) parts <- c(parts, list(Xb))
  if (!is.null(Xc) && nrow(Xc) == length(sam) && ncol(Xc) > 0) parts <- c(parts, list(Xc))
  design <- do.call(cbind, parts); storage.mode(design) <- "double"
  ...
  if (!"x" %in% colnames(design)) stop("設計矩陣去共線後缺少 x 係數")
...
collect_all_long_tp53 <- function(dataset_ids, strata, versions, groups, stat_tags, subunits){
  ...
  for (ds in dataset_ids)
    for (stt in strata)
      for (verx in versions)          # ← 避免與外層同名變數陰影
        for (g in groups)
          for (st in stat_tags)
            for (su in subunits) {
...
for (ver in versions) {
  ...
  ## 依版本設定正確的 stat_tags（修正點 #2）
  stat_tags_ver <- if (ver == "RAW") {
    c("GSEA_limma_t_cont")  # RAW 只有 limma 連續
  } else {
    c(
      "GSEA_limma_t_cont",
      "GSEA_spearman",

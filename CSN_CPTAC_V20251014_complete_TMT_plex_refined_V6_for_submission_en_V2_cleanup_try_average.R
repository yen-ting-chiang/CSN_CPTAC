...
build_csn_score_safe <- function(mat0, subunits, combine_7AB = TRUE,
  ...
  # CSN_SCORE = Average of Z-scores of subunits (Fallback consistent with main)
  sc <- colMeans(X, na.rm = TRUE)

  out <- out_na
  out[names(sc)] <- as.numeric(sc)
  if (exists("log_msg", mode = "function")) {
    log_msg(
      "[CSN_SCORE-safe] fallback (Average)：genes=%d；nonNA=%d/%d",
      nrow(X), sum(is.finite(out)), length(out)
    )
  }
  out
}
...
build_csn_score <- function(mat0,
  ...
  if (length(keep_sam) > 0) {
    # CSN_SCORE = Average of Z-scores of subunits
    # (Original PCA logic removed)
    sc <- colMeans(X[, keep_sam, drop = FALSE], na.rm = TRUE)
    s[keep_sam] <- sc
  }
...
GSEA_PREFIX <- "proteomic_GSEA_average"
